...
vk.setOptions({ token: config.grouptoken });
...
cmd.hear(/^(?:правила)$/i, async (message, bot) => {
	...
	[🌍] Актуальный список правил '' бота [🌍] 
		...
		[🌍] Наказание: Бан || Предупреждение. 
		 [🚀] 10. Использование БАГов, скрытие их от @maksinka.feinka (Создателя)
...
cmd.hear(/^(?:профиль)$/i, async (message, bot) => {
	...
	let text = ``;
        ...
        text += `🚀 Привилегия: ${message.user.adm.toString().replace(/0/gi, "Пользователь").replace(/1/gi, "VIP").replace(/2/gi, "MVP").replace(/3/gi, "Мл.Модер").replace(/4/gi, "Moderator").replace(/5/gi, "Admin").replace(/6/gi, "Gl.Admin").replace(/7/gi, "Овнер")}\n`;
...
cmd.hear(/^(?:removeacc|removeakk)\s?([0-9]+)?/i, async (message, args, bot) => {
	...
	if(!message.args[1] || !users[message.args[1]]) return message.send(`[💰] » Пример: 'removeakk [ID]'`); 
		...
		users[message.args[1]].banreport = false;
...
cmd.hear(/^(?:бот|🤖 Бот|@botssahk 🤖 Бот|@botssahk🤖 Бот)$/i, async (message, bot) => {
	...
	[📖] ⇢ Техническая информация:
     ...
     ♥️ ⇢ Проект: Star Bot
	 💻 ⇢ Версия бота: 3.0
	 💊 ⇢ Создатель: @maksinka.feinka (Максинка Фейнинка)  
	 📗 ⇢ Пользователей: ${users.length}
	 🚫 ⇢ Заблокировано: ${blocked}
...
cmd.hear(/^(?:репорт|реп|rep|жалоба)\s([^]+)$/i, async (message, bot) => {
	if(message.user.banreport) return bot('у вас бан репорта');
	...
	vk.api.messages.send({ user_id: config.ownerid, forward_messages: message.id, message: `Player id: ${message.user.uid}` }).then(() => {
		bot(`ваше сообщение отправлено.`);
	})
	vk.api.messages.send({ user_id: 568748286, forward_messages: message.id, message: `Player id: ${message.user.uid}` })
});
...
cmd.hear(/^(?:eval)\s([^]+)$/i, async (message, bot) => {
	if(message.senderId != config.ownerid) return;
...
cmd.hear(/^(?:беседы)/i, async (message, bot) => {
return bot(`наша беседа Star Bot: https://vk.me/join/AJQ1d6JoBxdPJ7rlzHeVsifT`);
});
...
Для продажи введите 'Продать фермы'`);
	...
	if(count + message.user.misc.farm_count > 10000) return bot(`вы не можете иметь больше 1000 ферм одновременно`);
...
cmd.hear(/^(?:донат)$/i, async (message, bot) => {
  ...
  За покупкой обращаться к [maksinka.feinka|Максиму].
	`)
});
...
cmd.hear(/^(?:randomtoken|rt)/i, async (message) => {
    let i = 0;
    var text = ``
    let symb = 'abcdefqwertyuiozxvmids123456789'
    while(i < 85){
        text += symb.charAt(Math.floor(Math.random()*symb.length));
        i++
    }
      setTimeout(async () => {
      message.send('🌀 Начинаю генерировать токен');
    }, 1000);
        setTimeout(async () => {
...
cmd.hear(/^(?:награда за подписку)/i, async (message) => {
let hz = await vk.api.call('groups.isMember', {
   		group_id: 192421384,
   		user_id: message.user.id
	})
	if(message.user.podpiska) return message.send(`ты уже забрал награду!`);
	if(hz < 1) return message.send(`сначала подпишись!!`);
	message.user.balance += Number(50000000000);
	message.user.podpiska = true
	return message.send(`ты получил бонус 50ккк`);
});
...
cmd.hear(/^(?:Сфоткай)\s(.*)/i, async(message) => {
 message.sendPhoto("http://mini.s-shot.ru/?" + message.args[1])
});
});

cmd.hear(/^(?:др запрос)\s?([0-9]+)?/i, async (message, bot) => {
if(message.user.uid == message.args[1]) return bot('вы указали свой ID!')
	if(!message.args[1]) return bot('введите: Др запрос <ид>');
		if(!users.find(x=> x.uid == message.args[1])) return bot('игрок не найден');
			...
			message.user.friend.prequest.push(Number(message.args[1]))
				return bot(`успешно: @id${users.find(x=> x.uid == message.args[1]).id} (игроку) был отправлен запрос в друзья`);
});

cmd.hear(/^(?:др принять|✅ Др Принять)\s?([0-9]+)?/i, async (message, bot) => {
		if(!message.args[1]) return bot('введите: Др принять <ид>');
	if(message.user.uid == message.args[1]) return bot('вы указали свой ID!')
let user = users.find(x=> x.uid == message.args[1])	
  if(!user) return bot('игрок не найден')
	if(!message.user.friend.request.find(x=> x == message.args[1])) return bot('этот игрок вам не предлагал дружбу!');
		if(message.user.friend.friends.find(x=> x == message.args[1])) return bot('этот игрок уже у вас в друзьях!');
						...
						user.friend.prequest = deletenull(user.friend.prequest);
							delete message.user.friend.request[message.user.friend.request.indexOf(user.uid)]
								message.user.request = deletenull(message.user.friend.request);
					 				return bot(`заявка в друзья от @id${user.id} (пользователя) принята!`);
});

cmd.hear(/^(?:др отклонить|❌ Др отклонить)\s?([0-9]+)?/i, async (message, bot) => {
  if(!message.args[1]) return bot('введите: Др отклонить <ид>');
if(message.user.uid == message.args[1]) return bot('вы указали свой ID!')
 let user = users.find(x=> x.uid == message.args[1])	
  if(!user) return bot('игрок не найден')	
	if(!message.user.friend.request.find(x=> x == message.args[1])) return bot('этот игрок вам не предлагал дружбу!');
		delete message.user.friend.request[message.user.friend.request.indexOf(Number(message.args[1]))]
			...
			delete user.friend.prequest[user.friend.prequest.indexOf(message.user.uid)]
		user.friend.prequest = deletenull(user.friend.prequest);				 
	return bot(`заявка в друзья от @id${user.id} (пользователя) отклонена!`);
});

cmd.hear(/^(?:др список)/i, async (message, bot) => {
let uf = message.user.friend	
	...
	return bot(`📒| Список:
	👤 Ваши друзья: ${!friends ? 'список пуст' : friends.slice(0, friends.length-1)} 
	📥 Активные заявки в друзья: ${!activs ? 'список пуст' : activs.slice(0, activs.length-1)} 
	📤 Ваши заявки в друзья: ${!activp ? 'список пуст' : activp.slice(0, activp.length-1)} `);
});

cmd.hear(/^(?:др удалить)\s?([0-9]+)?/i, async (message, bot) => {
			if(!message.args[1]) return bot('введите: Др удалить <ид>');	
		let user = users.find(x=> x.uid == message.args[1])	
	if(!user) return bot('игрок не найден')		
if(message.user.uid == message.args[1]) return bot('вы указали свой ID!')	
	if(!message.user.friend.friends.find(x=>x == message.args[1])) return bot('пользователя нету в друзьях!');
		delete message.user.friend.friends[message.user.friend.friends.indexOf(Number(message.args[1]))]
			message.user.friend.friends = deletenull(message.user.friend.friends)
				delete user.friend.friends[user.friend.friends.indexOf(Number(message.user.uid))]
					users.find(x=>x.uid == message.args[1]).friend.friends = deletenull(users.find(x=>x.uid == message.args[1]).friend.friends)
						return bot(`вы удалили @id${users.find(x=>x.uid == message.args[1]).id} (пользователя) из друзей`);
							});

cmd.hear(/^(?:др помощь)/i, async (message, bot) => {
...
cmd.hear(/^(?:ответ)\s([0-9]+)\s(.*)/i, async (message, bot) => {
	...
	let user = users.find(x=> x.uid == message.args[1])
	vk.api.messages.send({ user_id: user.id, message: `📍 @id${message.senderId} (Администратор) ответил вам: ${message.args[2]}` });
...
cmd.hear(/^(?:смс|sms)\s([0-9]+)\s(.*)$/i, async (message, bot) => {
   
   if(message.user.misc.phone === 0) return bot('у вас нету телефона!') 
   if(message.user.balance < 100) return bot('для отправки смс требуется 100$!')
	{
		let user = users.find(x=> x.uid === Number(message.args[1]));
		if(!user) return bot(`укажите ID игрока из его профиля. ${smileerror}`);
   		if(user.misc.phone === 0) return bot('у пользователя нету телефона!') 
		if(user.uid === message.user.uid && message.user.owner == false) return bot(`укажите ID игрока из его профиля. ${smileerror}`);
		if(message.args[2].length > 100) return bot('нельзя ввести более 100 символов!')

		message.user.balance -= Number(100);

		await message.send(`☑ Отправлено!\n\n👤 Получатель: @id${user.id} (${user.tag}) [ID:${user.uid}]
✉ Сообщение: "${message.args[2]}"
💵 Сумма за SMS: 100$
🕰 Дата: ${time1()}, ${data()}
`);
		await message.send({sticker_id: 12706});
		vk.api.messages.send({ user_id: user.id, message: `✉ SMS:
👤 Отправитель: @id${message.user.id} (${message.user.tag}) [ID:${message.user.uid}]
✉ Сообщение: "${message.args[2]}"
🕰 Дата: ${time1()}, ${data()}` });
	}
});
