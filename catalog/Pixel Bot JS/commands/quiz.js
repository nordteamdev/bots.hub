...
module.exports = {
	...
	f: function (msg, args, vk, bot){
	   ...
	   if(args[1].toLowerCase() == 'начать'){
           ...
           📖 | Вопрос: ${q.answer}
           🎁 | Приз: ${q.price} коинов
           📢 | Чтобы дать ответ : Эд викторина ответ [слово or число] (без [])
           💡 | Подсказка: Nik викторина подсказка. Подсказка стоит ${Math.floor(q.price / 2)}.
           `})
       }
       ...
       if(args[1].toLowerCase() == 'подсказка'){
           ...
           bot({text: `💵 | С вашего счета списано ${Math.floor(q.price / 2)} коинов за подсказку
