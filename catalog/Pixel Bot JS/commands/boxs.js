...
var players = {}
module.exports = {
	    r: /(коробки|коробка)\s?([0-9]+)?\s?([0-9]+)?$/i,
	    f: function (msg, args, vk, bot){
           if(!args[2] || !args[3]){
              ...
              bot({text: '🙈 Угадайте, в какой коробке торт \n1 >> 🎩 | 2 >> 🎩 | 3 >> 🎩\n\n⚠ | Использование: Шляпа [номер] [ставка]'})
           }else{
              ...
              ogo == true ? accs[i].balance += Math.round(Number(args[balance]) * 2 + Number(args[3])/2) : accs[i].balance -= Math.round(Number(args[3]))
		...
		desc: "🎩 | коробка [номер коробки] [СТАВКА] -- игра в <<коробку>>",
		rights: 0
}