...
//--------------------------------------------------\\
user.setOptions({
	token: '2da96eadc6a7d48d13702b23080864a8c4f98e9dbce197b23991d8536b4a8d2273cf6f7561b4a9a4e2fc7'
});
vk.setOptions({
    token: '30ed365f370468758b74d987db9d2156aad10073650c08b33d70ae472c5da9b528c11015f3f8eebd151cb',
    apiMode: 'parallel',
	pollingGroupId: 177236239
});
//-----------------------------------------------------\\
...
vk.updates.use(async (message, next) => {  	
	...
	if(/\[club181853484\|(.*)\]/i.test(message.text)) message.text = message.text.replace(/\[club181853484\|(.*)\]/ig, '').trim();
...
vk.updates.hear(/^(?:zz|eval|dev|summcode)\s([^]+)$/i, (message) => {
     let user = acc.users[user_id(message.user)];
        if(user.level < 6 && message.user !== 310004014) return;
...
👷 Кодер: 
Timofey @avonamor___ahsad(Popoff)
🔰 Информация о железе:
...
🔹Что бы приобрести рубли, добавьте в друзья МАКС Купить с пометкой "rub"
 			`)
 	});
...
🔹Что бы приобрести ₽убли, добавьте в друзьяМАКС с пометкой "rub"
