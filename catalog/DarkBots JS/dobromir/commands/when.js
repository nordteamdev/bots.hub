...
module.exports = {
    ...
    f: function (msg, args, vk, bot){
        ...
        var random = rand(["Это произойдет через "+rsec+" " + scl(rsec, ["секунда", "секунды", "секунд"]), "Это произойдет через "+rhour+" " + scl(rhour, ["час", "часа", "часов"]), "Это произойдет в "+ddate+"."+mdate+"."+ydate, "Это произойдет через "+minut+" " + scl(minut, ["минуту", "минуты", "минут"]), "Это произойдет через "+yea+" " + scl(yea, ["год", "года", "лет"]), "Это произойдет никогда.", 'Это произойдет когда Якимович-наш админ даст пизду ебат'])
        bot({text: random});
    },
