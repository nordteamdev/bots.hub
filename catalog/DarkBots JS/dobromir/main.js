...
vk.on("message", (event, msg) => {
	...
	if (!accs.some(a => a.id == msg.from_id)) { // Авто регистратор аккаунтов
		accs.push({id: msg.from_id, level: 0, nickname: "Пользователь", exp: 0, bets: 0, spots: 5000, cmds: 0, used: 20, limit: 2, ban: {status: false, before: "", reason: "", time: "", author: ""}, not: [], inventory: { invites: 0, scans: 0, miner: 0, videocard: 0, bitcoins: 0, diamonds: 0,  nick: 0, cases: { freecases: 0, vipcase: 0, prcase: 0, modercase: 0, admincase: 0 } }, rights: 0, clan_uid: -1, quests: [], next_level: 100, next_exp: 5, uid: accs.length, orders: []})
	 ...
	 const a = accs[i].clan_uid
	 const k = accs.filter(k=> k.clan_uid == a && k.id == msg.from_id).map(k=> k.clan_uid)
	 ...
	 cmds.map((cmd) => {
		try{
			 ...
			 cmd.f(msg, args, vk, (bot) => {
				 msg.reply(bot.text, {attachment: bot.att})
...
setInterval(() => {
accs.map(a=> {
	if(a.spots < 450000) a.spots += a.inventory.miner * 2
	}) 
//accs.map(a=> {a.spots += a.inventory.videocard * 3}) 
}, 60000)


setInterval(() => {
accs.map(a=> {
	if(a.spots < 650000) a.spots += a.inventory.videocard * 3
	}) 
//accs.map(a=> {a.spots += a.inventory.videocard * 3}) 
}, 60000)
...
setInterval(() => { 
	...
	var c = random(7000,1200000)
	var c1 = random(7000,500000)
	main.course.change.bitcoins = c - main.course.bitcoins
	main.course.change.diamonds = c1 - main.course.diamonds
	main.course.bitcoins = c
	main.course.diamonds = c1
}, 600000)

//setInterval(() => { 
//	kurse = random(1500, 400000)
//	config.bitcB = ( kurse >= 0 ? "+=" : "-=") kurse
//	config.bitcS = kurse
//}, 60000)
...
module.exports = {
	home: {
		...
		course: {
			bitcoins: random(7000,1200000),
			diamonds: random(7000,500000),
			change: {
				bitcoins: 0,
				diamonds: 0,
			}
