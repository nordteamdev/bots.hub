...
function antispam(){
	...
	/*for(var s = 0; s < Number(users.length); s++){ 
        ...
        if(user >= 40){
            var reasons = ["Не строчи так много!", "Нарушение правила №1.2 | Подробнее тут --> http://bot.cryazbotss.online/rules"]
...
function ban(array){
    ...
    sendvk({text: text_user, id: array.id, att: "photo-137139998_456239538"})
    return lolik
}
...
function unban(array){
    ...
    sendvk({text: text_user, id: id, att: "photo-137139998_456239540"})
    return lolka
}
function forbans(){
