...
module.exports = {
	    ...
	    f: function (msg, args, vk, bot){
           ...
           if(spot == 0) return
		   if(spot > mes.limit) return bot({text: "Превышен лимит ставки. Макс лимит - "+ mes.limit +" поинтов."})
		   if(accs[i].balance < spot) return bot({text: "Ты ставишь больше, чем свой баланс."})
		   if(accs[i].balance <= 0) return bot({text: "Извини, но у тебя " + accs[i].balance + " поинтов."})
