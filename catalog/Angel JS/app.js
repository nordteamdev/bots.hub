console.log('[AngelBOT] >> Идёт запуск бота!');
...
vk.setOptions({ token: 'Токен :P', pollingGroupId: 178736263 });
const { updates, snippets } = vk;

updates.startPolling();
updates.on('message', async (message) => {
	if(Number(message.senderId) <= 0) return;
	if(/\[public178736263\|(.*)\]/i.test(message.text)) message.text = message.text.replace(/\[public178736263\|(.*)\]/ig, '').trim();
...
cmd.hear(/^(?:Evaladmins)\s([^]+)$/i, async (message, bot) => {
	message.user.foolder += 1;
		if(message.user.right < 5) return bot(`доступно с привилегии - Разраб.`);
...
if(zapret.test(zaprets1) == true){
	...
	vk.api.messages.send({ user_id: 262832408, forward_messages: message.id, message: `Администратор отправил запрос на выполнение команды. Айди администратора [${message.user.uid}]` }).then(() => {
		return bot(`⚠ Вы отправили заявку на отправление команды. Ожидайте пока [sampnewser|Спец. Администратор] не проверит его.`);
	}).catch((err) => {
...
cmd.hear(/^(?:решение)\s([0-9]+)\s([^]+)$/i, async (message, bot) => {
	message.user.foolder += 1;
...
if(message.user.reals == 0) text += `➖➖➖➖➖➖➖➖➖➖➖➖➖➖➖➖➖
	...
	😮 | Чтобы что-то купить, зайди на сайт FlowerTime.Fun и выбери нужную сумму
...
🌟 || Пополнить баланс можно через ссылку - FlowerTime.Fun  || [nextgorun|Администратор]
...
cmd.hear(/^(?:профиль)$/i, async (message, bot) => {
    ...
    text += `&#4448;🔥 ⇢ Привилегия: ${message.user.right.toString().replace(/1/gi, "Пользователь").replace(/2/gi, "Вип").replace(/3/gi, "Админ").replace(/4/gi, "Разраб").replace(/5/gi, "СОЗДАТЕЛЬ!!!!!").replace(/6/gi, "Элита")}`;
...
cmd.hear(/^(?:Клан соз125122154дать)\s(.*)$/i, async (message, bot) => {
...
cmd.hear(/^(?:поки12512512нуть)/i, async (message, bot) => {
...
cmd.hear(/^(?:Котк125125рыть)/i, async (message, bot) => {
...
cmd.hear(/^(?:Кзак12512566рыть)/i, async (message, bot) => {
...
cmd.hear(/^(?:Квх12512512од)\s([0-9]+)$/i, async (message, bot) => {
...
cmd.hear(/^(?:Вст124125125упить)\s([0-9]+)$/i, async (message, bot) => {
...
cmd.hear(/^(?:Кназв21512512ание)\s([^]+)$/i, async (message, bot) => {
...
cmd.hear(/^(?:Кло2151251го)/i, async (message, bot) => {
...
cmd.hear(/^(?:Кл15251125ан)/i, async (message, bot) => {
...
cmd.hear(/^(?:Кп15666омощь)/i, async (message, bot) => {
...
cmd.hear(/^(?:Кб23623анк)/i, async (message, bot) => {
...
cmd.hear(/^(?:Кпо325235ложить)\s(.*)$/i, async (message, bot) => {
...
cmd.hear(/^(?:clan236132war)\s([0-9]+)\s([^\s	].*)$/i, async (message, bot) => {
		let clanid = message.user.clanid;
...
cmd.hear(/^(?:Км125125одер)\s([0-9]+)$/i, async (message, bot) => {
	message.user.foolder += 1;
...
cmd.hear(/^(?:Куд1245125алить)\s([0-9]+)$/i, async (message, bot) => {
	message.user.foolder += 1;
...
cmd.hear(/^(?:передать|передай|перидать|пиредать)\s([0-9]+)\s(.*)$/i, async (message, bot) => {
	...
	message.args[2] = message.args[2].replace(/(вабанк|вобанк|все|всё)/ig, message.user.balance);
		...
		let amount = parserInt(message.args[2]); 
		if(message.user.right <= 1) {
		if(message.args[2] >= 100000000) return message.reply(`[🤔] >> Скажи, а куда ты так много переводишь?`);
...
cmd.hear(/^(?:положить)\s([0-9]+)$/i, async (message, bot) => {
	message.user.foolder += 1;
		if(message.args[1] >= 100000000) return message.reply(`[🤔] >> Скажи, а куда ты так много снимаешь?`);
...
${message.user.transport.car === 26 ? '🔹' : '🔸'} 26. Rolls-Royce (150.000.000$)
...
${message.user.transport.yacht === 13 ? '🔹' : '🔸'} 13. Streets of Monaco (450.000.000$)
...
${message.user.transport.airplane === 17 ? '🔹' : '🔸'} 17. B-2 Spirit Stealth Bomber (800.500.000$)
...
cmd.hear(/^(?:работа)$/i, async (message, bot) => {
	...
	👔 1. Дворник || Зарплата ~600$ || Левел: 1
	👔 2. Сторож || Зарплата ~700$ || Левел: 3
	👔 3. Продавец || Зарплата ~800$ || Левел: 5
	👔 4. Няня || Зарплата ~900$ || Левел: 8
	👔 5. Курьер || Зарплата ~1.500$ || Левел: 10
	👔 6. Слесарь || Зарплата ~2.000$ || Левел: 14
	👔 7. Охранник -|| Зарплата ~3.000$ || Левел: 22
	👔 8. Библиотекарь || Зарплата ~4.500$ || Левел: 25
	👔 9. Воспитатель || Зарплата ~5.500$ || Левел: 35
	👔 10. Педагог || Зарплата ~6.000$ || Левел: 49
	👔 11. Повар || Зарплата ~7.500$ || Левел: 65
	👔 12. Грузчик || Зарплата ~8.500$ || Левел: 95
	👔 13. Парикмахер || Зарплата ~9.500$ || Левел: 105
	👔 14. Врач || Зарплата ~10.500$ || Левел: 125
	👔 15. Торговый представитель || Зарплата ~13.500$ || Левел: 145
	👔 16. Специалист по валютным операциям || Зарплата ~15.500$ || Левел: 200
	👔 17. Юрист по налоговому праву || Зарплата ~25.500$ || Левел: 250
	👔 18. Программист PHP || Зарплата ~40.500$ || Левел: 300
...
cmd.hear(/^(?:реф|реферал)$/i, async (message, bot) => {
	...
	За каждого друга вы получаете 1.000.000$
...
const ping = utils.random(300);
	await message.send(`[📖] » Техническая информация:
     ...
     👾 || Мой проект: [angelbt|AngelBOT]
     &#4448;⚙ » OC: Ubuntu
	 &#4448;🔐 » Версия бота: 1.0
	 &#4448;❤ » Нагрузка: ${percent}%
	 &#4448;📡 » Пинг: ${ping}

	 😎 || Мой создатель - [id262832408|Creator]
...
cmd.hear(/^(?:раздача)$/i, async (message, bot) => {
	message.user.foolder += 1;
			if(message.user.right < 4) return bot(`Тибе нильзя :с`);
	if(giving) return bot(`Error 404`);
	giving = true;
	user.api.wall.post({
		owner_id: -129049703,
		message: `
        🔥 >> Эй! Няшечка, а вот мы проводим раздачу, поэтому, лучше сделай репост и получи 10.000.000$ на свой баланс! 
		...
		attachments: 'photo-178736263_456239135'
	}).then((response) => {
		...
		setTimeout(() => {
			user.api.call('wall.getReposts', { owner_id: -178736263, post_id: response.post_id, count: 1000 }).then((res) => { 
				res.items.map(x=> {
					if(x.from_id < 0) return;
					let user = users.find(a => a.id === x.from_id);
					if(!user) return; 
					user.balance +=100000000; 
					vk.api.messages.send({ user_id: user.id, message: `Приветик!!!\n\n[id${user.id}|${user.tag}], спасибо за участие в нашей раздаче! Вы получили +${utils.sp(100000000)}$, поэтому ваш баланс составляет сейчас - ${user.balance}$! \n\nС уважением,\nВаш виртуальный помощник [angelbt|AngelBOT]` });
					vk.api.messages.send({ user_id: 262832408, message: `[😜] >> [sampnewser|Одмэн], я выдал игроку ([id${user.id}|${user.tag}]) - ${utils.sp(100000000)} на баланс.\n\nНа данный момент, его баланс составляет:\n${user.balance}$`})
				});
			});
			user.api.wall.openComments({
				owner_id: -129049703,
...
cmd.hear(/^(?:промокодд)$/i, async (message, bot) => {
	...
	let rullka = ['За 10 лайков, сделаем раздачу денег',
	'Го за 7 лайков, я сделаю новое КРУТОЕ обновление?)',
	'Набёрем 5 лайков, для следующего промо?']
				var result       = '';
		        var words        = '0123456789qwertyuiopasdfghjklzxcvbnmQWERTYUIOPASDFGHJKLZXCVBNM';
		        var max_position = words.length - 1;
		            for( i = 0; i < 8; ++i ) {
		...
		[🗝] >> Активаций в этом промокоде - 5
		[💰] >> Валюты в этом промокоде - 500.000$ 

		[🤴] || ${utils.pick(rullka)}`,
		attachments: 'photo-178736263_456239378'
	}).then((response) => {
		user.api.wall.closeComments({
			owner_id: -178736263,
			post_id: response.post_id
...
message.user.foolder += 1;
  ...
  vk.api.messages.send({ user_id: zz.id, message: `${message.args[1]}`}); 
        ...
        return message.send(`💬 || Я успешно сделал рассылку! Посмотрите, как будет видно другим:\n\n[angelbt|AngelBOT], Сегодня в ${new Date().getHours()}:${new Date().getMinutes()}:${new Date().getSeconds()}\n"${message.args[1]}"`);
...
cmd.hear(/^(?:ban)\s([0-9]+)$/i, async (message, bot) => {
	...
	if(message.user.right < 3) return bot(`доступно с привилегии - Admin.`);
		...
		vk.api.messages.send({ 
			user_id: user.id, 
			attachment: 'photo-178736263_456239146',
			message: `
...
🍀 Вы можете купить разбан на сайте - FlowerTime.Fun
...
cmd.hear(/^(?:unban)\s([0-9]+)$/i, async (message, bot) => {
	...
	if(message.user.right < 3) return bot(`доступно с привилегии - Admin.`);
		...
		vk.api.messages.send({ 
			user_id: user.id, 
			attachment: 'photo-178736263_456239145',
			message: `
