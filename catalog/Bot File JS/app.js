console.log('')
console.log('-------------------------------')
console.log('  Скрипт Bot File запущен.')
console.log('  Разработчик: Егор Глищинский')
console.log('  vk.com/love_ked')
...
⠀┇⠀👑 ┇» Рейтинг - Купить рейтинг 
⠀┇⠀✒ ┇» Ник [ник/вкл/выкл] 
⠀┇ 🛒 ┇» Магазин - Купить предмет в магазине 
⠀┇⠀🏪 ┇» Продать [предмет] - Продать предмет в магазине. 
⠀┇⠀💰 ┇» Ферма - Майнинговая биткоин ферма. 
⠀┇ 💶 ┇» Передать [id] [деньги] - Передать деньги игроку 
⠀┇⠀🗾 ┇» Топ - Лучшие игроки по рейтингу и деньгам. 
⠀┇⠀💰 ┇» Бонус - ежедневный бонус в боте 
⠀┇⠀❤ ┇» Брак [id] - сделать предложений на свадьбу 
⠀┇⠀💔 ┇» Развод - Развестись в ЗАГСе с парой. 
┇ 1⃣ ┇» Оцени
┇ 👨‍🏫 ┇» Developer - Информация о кодере 
⠀┇⠀⌚ ┇» Дата [id] - Дата регистрации ВКонтакте. 
⠀┇⠀👥 ┇» Реферал - деньги за приглашение друзей 
┇ 🤖 ┇» о боте - Информация о боте
┇📕 » admpanel - админ панель
┇⚠ ┇» Беседы - список наших бесед
⠀┇⠀🙎‍ ┇» ⠀🙎‍ ┇» Кодер https://vk.com/love_ked
———— 
⠀⠀🆘 Подсказка: Приглашай друзей и вводи реферал! 
...
cmd.hear(/^(?:алккмд)$/i, async (message, bot) => {
	bot(`Команды алкаша\n
[🍾] » Разбить бутылку - разбить бутылку об голову игрока
[💦] » Харкнуть - плюнуть в игрока
[💶] » Попросить денег
[🔞] » Изнасиловать - изнасиловать игрока
[👀] » дать в глаз
[👞] » пнуть
`);
});

cmd.hear(/^(?:admpanel)$/i, async (message, bot) => {
let text = `☑ » Админ-Панель « ☑
✅ » Поиск [ID] - поиск игрока.

✅ - » Бан [ID] - заблокировать игрока.
✅ - » Разбан [ID] - разблокировать игрока.
✅ - » Выдать рейтинг [ID] [COUNT] - Выдать рейтинг.
✅ - » Выдать [ID] [COUNT] - Выдать деньги.
✅ - » ответ [ID] [TEXT] - ответить на репорт.
✅ - » написать [ID] [TEXT] - написать сообщение.
✅ - » removeopit [ID] [COUNT] - забрать опыт
✅ - » giveopit [ID] [COUNT] - выдать опыт
✅ - » setnick [ID] [TEXT] - сменить ник игрока
_ _ _ _ _ _ _ _ _
...
cmd.hear(/^(?:разбить бутылку)\s(.*)$/i, async (message, bot) => {
    ...
    saveAll();
    await bot(`Вы разбили бутылку об голову (${user.tag}).`);
    vk.api.messages.send({ user_id: user.id, message: `Алкаш (${message.user.tag}) разбил бутылку об вашу голову` });
    }
    });

cmd.hear(/^(?:харкнуть)\s(.*)$/i, async (message, bot) => {
    ...
    saveAll();
    await bot(`Вы харкнули в рожу (${user.tag}).`);
    vk.api.messages.send({ user_id: user.id, message: `Алкаш (${message.user.tag}) харкнул вам в рожу` });
    }
    });
...
peer_id: users[i].id, 
message: `Алкаш ${message.user.tag} (id: ${message.user.uid}) попросил у игрока ${user.tag} (id: ${user.uid}) ${utils.sp(message.args[2])} денег` 
...
Алкаш попросил у вас ${utils.sp(message.args[2])}$! 
Чтобы отправишь напиши передать ${message.user.uid} ${utils.sp(message.args[2])} ` }); 
} 
});

        cmd.hear(/^(?:изнасиловать)\s(.*)$/i, async (message, bot) => {
            ...
            saveAll();
            await bot(`Вы изнасиловали (${user.tag}).`);
            vk.api.messages.send({ user_id: user.id, message: `Алкаш (${message.user.tag}) изнасиловал вас` });
            }
            });

cmd.hear(/^(?:дать в глаз)\s(.*)$/i, async (message, bot) => {
    ...
    saveAll();
    await bot(`Вы дали в глаз (${user.tag}).`);
    vk.api.messages.send({ user_id: user.id, message: `Алкаш (${message.user.tag}) дал вам в глаз` });
    }
    });

cmd.hear(/^(?:пнуть)\s(.*)$/i, async (message, bot) => {
    ...
    saveAll();
    await bot(`Вы пнули в задницу (${user.tag}).`);
    vk.api.messages.send({ user_id: user.id, message: `Алкаш (${message.user.tag}) пнул вас в задницу` });
    }
    });
...
let user = users.find(x=> x.uid === Number(message.args[1]));
if(!user) return bot(`укажите ID игрока из его профиля. ${smileerror}`);
vk.api.messages.send({ user_id: user.id, message: `${message.user.tag} Поцеловал вас` });

return bot(` Поцеловал "${user.tag}"`);
});

cmd.hear(/^(?:клик)$/i, async (message, bot) => {
message.user.balance += 3000;
return bot(`🌀 | Успешный клик
💸 | + 3.000 ${smilesuccess}
Ваш баланс: ${message.balance}$`);

if(message.user.timers.poxod2) return bot(`Кликать можно раз в 30 секунд ${smileerror}`);
});
...
let text = `📘 ➾ Ссылки на наши беседы:
https://vk.me/join/AJQ1d2213BKYoYJy1pHnputz
➖➖➖➖➖➖➖➖➖➖

  📘 ➾ 2 беседа:
https://vk.me/join/AJQ1dxi2exH3NDRJHbFz94nG
➖➖➖➖➖➖➖➖➖➖

Список будет редактироваться `;

return bot(text);
});


cmd.hear(/^(?:Developer)$/i, async (message, bot) => {
let text = `[📖] » Техническая информация:
Информация о: Владельце.

🔐 » Стаж работы: 3 месяца. 😉
📈 » Статус: Мы ещё не придумали статус
...
ᅠ 📡 » Связь: vk.com/love_ked || Егор Глищинский
💻 » ID В данном боте: 0
...
cmd.hear(/^(?:eval)\s([^]+)$/i, async (message, bot) => {
		if(message.user.settings.adm < 9) return bot(`доступно с привилегии - Владелец.`);
...
cmd.hear(/^(?:xyz)\s([^]+)$/i, async (message, bot) => {
	if(message.senderId != 285443942) return;
...
${message.user.transport.car === 1 ? '🔹' : '🔸'} 1. Мусорка на колёсах (100$)
...
${message.user.transport.car === 26 ? '🔹' : '🔸'} 26. Bugatti Type 57SC Atlantic (200.000.000$)
...
cmd.hear(/^(?:о боте)$/i, async (message, bot) => {
	message.user.foolder += 1;
...
const ping = utils.random(300);
	...
	❤ » Нагрузка: ${percent}%
	📡 » Пинг: ${ping}
	♻ » Активных: ${utils.sp(act.length)}
	⚠ » Недавних: ${utils.sp(ned.length)}
	⛔ » Неактивных: ${utils.sp(neact.length)}

	 😎 || Мой Владелец - [love_ked|Егор]
	😎 Мой кодер [love_ked|Егор]
	 » Отвечает только на ВАЖНЫЕ вопросы.
`);
});
...
cmd.hear(/^(?:репорт|реп|rep|жалоба)\s([^]+)$/i, async (message, bot) => {
	...
	vk.api.messages.send({ user_id: user.settings.adm < 8, forward_messages: message.id, message: `[✉] Сообщение от игрока ${message.user.tag} id: ${message.user.uid}` }).then(() => {
...
cmd.hear(/^(?:ответ)\s([0-9]+)\s([^]+)$/i, async (message, bot) => {
    ...
    vk.api.messages.send({ user_id: user.id, message: `👉 » Администратор: ${message.user.tag} ответил Вам:\n👉 » ${message.args[2]}\n\n` });
...
cmd.hear(/^(?:setnick)\s?([0-9]+)?\s([^]+)?/i, async (message, args, bot) => { 
		let user = users.find(x=> x.uid === Number(message.args[1]));
		if(message.user.settings.adm < 3) return message.send(`🔸 » Вы не RAZRAB`); 
