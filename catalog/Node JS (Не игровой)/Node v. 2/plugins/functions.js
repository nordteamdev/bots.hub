...
function getRights(uuid) {
	...
	var r = accs[i].rights
	        return r == 5 ? '🔧Президент🔧' : (r == 4 ? 'Гл.Администратор' : (r == 3 ? 'Администратор' : (r == 2 ? 'Модератор' : (r == 1 ? 'Вип' : 'USER'))))
}
...
function dostup(num) {
	return num == 5 ? '🔧Президент🔧' : (num == 4 ? 'С Гл.Администратора' : (num == 3 ? 'С Администратора' : (num == 2 ? 'только Модераторам' : (num == 1 ? 'c Випки' : undefined))))
}
