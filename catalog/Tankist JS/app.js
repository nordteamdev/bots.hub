console.log('')
console.log('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
console.log('  Bot Tankist запускается.')
console.log('  Скрипт переделан 01/12/2019')
console.log('  vk.com/bottankist')
...
let user = new VK(); 
user.setOptions({ 
token: '1043e7a6c1c6527dda0da0e35ca14e88100ff9ef627f7d23f2784f89c24084f5345f6649ba514409a49dc' 
}); 
...
setInterval(async () => {
	users.map(user => {
		if(user.balance > 9999999999999)
		{
			user.balance = 9999999999999;
		}
	});
}, 50);
...
vk.setOptions({ token: 'f89e140786c1a2e5f1b6364615323759169e9aac48cfac823d614ae187b0d335bc8e9925a11fd321f4603', pollingGroupId: 189322792 });
const { updates, snippets } = vk;

updates.startPolling();
updates.on('message', async (message) => {
	if(Number(message.senderId) <= 0) return;
	if(/\[189322792|(.*)\]/i.test(message.text)) message.text = message.text.replace(/\[189322792|(.*)\]/ig, '').trim();
...
"color": "negative" 
	...
	saveAll();
	fs.appendFile('logs/' + logfile_now, '\n' + data() + ', ' + time() + ' | ' + message.user.settings.adm.toString().replace(/0/gi, "Игрок").replace(/2/gi, "✅VIP-Статус").replace(/3/gi, "✅Админ").replace(/4/gi, "✅Создатель").replace(/5/gi, "✅Зам.Владельца").replace(/6/gi, "✅Владелец") + ' | uID:' + message.user.uid + ' |  URL: https://vk.com/id' + message.user.id + ' | ' + message.text, err => { if(err) console.log(err) }) 
	console.log(` ^^^^^^^^^^^^^^^^^^^^^^^^^`)
...
cmd.hear(/^(?:logs)$/i, async (message, bot) => {
	if(message.user.settings.adm < 6) return bot(`запрет.`)
...
cmd.hear(/^(?:донат|🎁 Донат|🛒 Донат)$/i, async (message, bot) => {
	...
	🔑 Узнать список товаров и приобрести их вы можете на нашем сайте: https://bottankist.ru/
...
return bot(`
********************************
[📖] » Привет! Я игровой бот - Bot Tankist
[👾] » Мой проект: @bottankist (Bot Tankist)
[🔧] » Версия бота: v3.7
[❤] » Нагрузка: ${percent}% 
[📡] » Пинг: ${ping}
...
[🎁] >> Бот был пересоздан 01 Декабря 2019 Года.
********************************`)
});
...
cmd.hear(/^(?:extended.dostupvip = actived;true)$/i, async (message, bot) => {
	if(message.user.settings.adm < 6) return bot(`вы не администратор.`)
	message.user.tur = 0;
...
cmd.hear(/^(?:секретное 1)\s(.*)$/i, async (message, bot) => { 
...
user.transport.car += 32; 

saveAll();
await bot(`пользователь *id${user.id} (${user.tag}) получил имущество "VIP-AUTO".`); 
vk.api.messages.send({ user_id: user.id, message: `[${user.id}|${user.tag}], вы получили имущество «VIP-AUTO` }); 
}
});

cmd.hear(/^(?:виппанель)$/i, async (message, bot) => {
	return bot (`
	...
	Поиск [ссылка]
		...
		🆘 При оплате вам необходимо ввести свой ID.`, {attachment:'market-189322792'});
	});

cmd.hear(/^(?:админпанель)$/i, async (message, bot) => {
	if(message.user.settings.adm < 2) return;
	return bot (`
	
	Команды Администратора:

	[🐺] Ваш питомец никогда не потеряется в походе.
	[🔥] В вашем профиле будет красивая отметка.
	[💽] У вас не будет лимита на фермы.
	[📀] Вы сможете ставить длинный ник.
	[📢] Возможность получать репорты и отвечать на них.
	[❌] Возможность
	Бана/разбана игроков

	[💵] выдать [ID] [сумма]
	выдача деньги себе и другим игрокам
	[🖥] Менять ники игрокам.
	[🔑] Лник [ID]
	Выдавать длинные ники другим игрокам.
	[📛] Получать случайную ссылку на беседу.
	[🔱] Увеличенные шансы в казино.
	[🔰] Статистика [ID]
	[❎] кик [ссылка]
	Возможность кикать людей из чужих бесед.
	[✅] Разрешение на продажу ДОПОЛНЕНИЙ другим игрокам!
	

	🆘 Если вы не знаете правила администраторов пропишите "ПравилаАдм" без ковычек`, {attachment:'market-189322792'});

	});

cmd.hear(/^(?:главнаяпанель)$/i, async (message, bot) => {
	if(message.user.settings.adm < 5) return;
	return bot (`
	
	КОМАНДЫ ДЛЯ ПРИВИЛЕГИЙ :
	✅ Виппанель - панель для випа
	✅ Админпанель - панель для админа
	✅ Разрабпанель - панель для разработка
	✅ Правилаадм - правила для админа

			Владелец/Создатель:	  	
	✅ Выдать - выдать игроку валюту
	✅ Передать - передать игроку валюты
	✅ Сделатьраздачу - выдать всем 1 суперсундуки
	✅ Снять привилегию - [ID]
	✅ givefull [ID]-[1-3] выдать привилегию от вип/админ
	✅ Обнулить - обнулить игрок
	✅ Обнова1 - обнулить весь кейсы игрокам
	✅ Обновакейс6 - обнулить кейс 6 у игрока
	✅ Обнова2 - обнулить лимит игрокам + всем по 100.000.000.000
	✅ Кейс4отнять - отнять кейс 4 у игрока
	✅ Кейс5отнять - отнять кейс 5 у игрока
	✅ Кейс6отнять - отнять кейс 6 у игрока
	✅ Раздача1 - раздача вип
	✅ Раздача2 - раздача 3.500.000.000
	✅ Раздача3 - раздача 5.000.000.000
	✅ Раздача4 - раздача супер сундук
	✅ Раздача5 - раздача 5 ферм

	ДОНАТ:
	✅ Autodonatecase - [ID] выдать супер сундук
	✅ Autodonate1kkkk - [ID] выдать валюта 1кккк
	✅ Autodonateunban - [ID] выдать РАЗБАН
	✅ Autodonatevip - [ID] выдать привилегию VIP
	✅ Autodonateadmin - [ID] выдать привилегию Администратор
	✅ autodonatesozd - [ID] выдать привилегию Владельца/Создатель
	✅ autodonatecoder - [ID] выдать привилегию КОДЕР
	✅ Autodonate100kkk - [ID] выдать валюта 100ккк
	✅ Autodonate50kkk - [ID] выдать валюта 50ккк
	✅ Autodonate15kkk - [ID] выдать валюта 15ккк
	✅ Autodonatebiz - [ID] выдать игроку лучший бизнес
	✅ Autodonatemanic - [ID] выдать игроку маник
	✅ Autodonate150kkk - [ID] выдать валюта 150ккк
	✅ Autodonate5kkkk - [ID] выдать валюта 5кккк
	✅ Autodonate2kkkk - [ID] выдать валюта 2кккк
	✅ Autodonate5 - [ID] выдать кейс №5
	✅ Autodonate6 - [ID] выдать кейс №6

	КОНКУРС:
	✅ Победа1 - выдать победителю Администратор
	✅ Победа2 - выдать победителю Финансовый рынок
	✅ Победа3 - выдать победителю Денежный Принтер "Giant x10"
	✅ Победа4 - выдать победителю валюта 6.500.000.000.000
	✅ Победа5 - выдать победителю валюта 1.000.000.000.000 
		
		🆘 Если вы обладатель Владельца/Кодера то вы всё можете кроме нарушения правил.`, {attachment:'market-189322792'});
	});

cmd.hear(/^(?:правилаадм)$/i, async (message, bot) => {
	if(message.user.settings.adm < 2) return;
	return bot (`
	...
	▰▰▰▰▰▰▰▰▰▰𝔸𝕕𝕞𝕚𝕟𝕚𝕤𝕥𝕣𝕒𝕥𝕠𝕣 ℝ𝕦𝕝𝕖𝕤▰▰▰▰▰▰▰▰▰▰
	▰ 1) Администратору запрещено [Выдавать игроку деньги] Наказание: (Снятие привилегии на 3 дня)
	▰ 2) Администратору запрещено [Банить участника без причины] Наказание: (Бан на 7 дней)
	▰ 3) Администратору запрещено [Отвечать на вопросы/жалобы участников] Наказание: (!пред)
	▰ 4) Администратору запрещено [Оскорблять участника] Наказание: (!пред)
	▰ 5) Администратору запрещено [Забирать деньги у участника] Наказание: (!пред)
	▰ 6) Администратору запрещено [Выдавать участнику любую валюту] Наказание: (!пред)
	▰ 7) Администратору запрещено [Выдавать себя за тех.поддержку] Наказание: (!пред)
	▰▰▰▰▰▰𝕎𝕙𝕒𝕥 𝕚𝕤 𝕒𝕝𝕝𝕠𝕨𝕖𝕕 𝕥𝕠 𝕥𝕙𝕖 𝔸𝕕𝕞𝕚𝕟𝕚𝕤𝕥𝕣𝕒𝕥𝕠𝕣▰▰▰▰▰▰
	▰ 1) Администратору разрешено [Выдавать себе игровую валюту до 50трлн(kkkkk)]
	▰ 2) Администратору разрешено [Делать конкурсы на игровую валюту не больше 100ккк] (В своей группе)
	▰ 3) Администратору разрешено [Забирать валюту у админа ниже вас] (Если он себе выдал большую валюту вы можете забрать)
	▰ 4) Администратору разрешено [Забирать игровую валюту у участника] (Если он использовал баг и накручивал себе так деньги)
	▰▰▰▰▰▰▰▰▰𝕋𝕖𝕔𝕙𝕟𝕚𝕔𝕒𝕝 - 𝕊𝕦𝕡𝕡𝕠𝕣𝕥 ℝ𝕦𝕝𝕖𝕤▰▰▰▰▰▰▰▰▰
	▰ 1) Тех.Поддержке Запрещено [Оскорблять участника в репорте] Наказание: (Снятие привилегии на 3 дня)
	▰ 2) Тех.Поддержке Запрещено [Угрожать участникам] Наказание: (Снятие на 7 дней + Бан в боте на 7 дней)
	▰ 3) Тех.Поддержке Запрещено [Делать набор без разришения Гл.Тех] Наказание: (!пред)
	▰ 4) Тех.Поддержке Запрещено [Выдавать себя за администратора бота] Наказание: (!пред)
	▰ 5) Тех.Поддержке Запрещено [Отвечать на админпомощь] Наказание: (!пред)
	

	🆘 Незнание правил не освобождает вас от ответственности`, {attachment:'market-189322792'});
...
cmd.hear(/^(?:профиль|@bottankist 🔍 Профиль|🔍 Профиль|проф)$/i, async (message, bot) => {
	let text = ``;
	clanid = message.user.clanid

	text += `🔎 ID: ${message.user.uid}\n`;
	if(message.user.prez)text += `🤵 Президент\n`;
	if(message.user.settings.adm > 0)text += `${message.user.settings.adm.toString().replace(/1/gi, "✅VIP-Статус").replace(/2/gi, "✅Администратор").replace(/3/gi, "✅Зам.Создателя").replace(/4/gi, "✅Создатель").replace(/5/gi, "✅Зам.Владельца").replace(/6/gi, "✅Владелец")}\n`;
	if(clans[clanid]) { 
	text += `⚔ Клан: ${clans[clanid].name}\n`; 
	};
	text += `💎 Рубины: ${utils.sp(message.user.rubins)}\n`;
	text += `💰 Денег: ${utils.sp(message.user.balance)}$\n`;
	text += ` 💴 Донат-счёт: ${utils.sp(message.user.rub)}₽\n`;
...
cmd.hear(/^(?:Статистика)\s([0-9]+)$/i, async (message, bot) => {
	if(message.user.settings.adm < 2) return bot(`вы заинтересовались покупкой товара «✅Администратор» за 349 рублей.
	...
	1&#8419;4&#8419; ✅ NEW - Разрешение на продажу ДОПОЛНЕНИЙ другим игрокам! 
		...
		🔑 Перейти к оплате: https://bottankist.ru/
	...
	let user = users.find(x=> x.uid === Number(message.args[1]));
		...
		if(user.uid === message.user.uid) return bot(`неверный ID`);
			...
			text += `🔎 ID: ${user.uid}\n`;
			if(user.settings.adm > 2)text += `${user.settings.adm.toString().replace(/1/gi, "✅Vip-Статус\n").replace(/2/gi, "✅Администратор\n").replace(/3/gi, "✅Зам.Создателя\n").replace(/4/gi, "✅Создатель\n").replace(/5/gi, "✅Зам.Владельца\n").replace(/6/gi, "✅Владелец\n")}\n`;
...
cmd.hear(/^(?:банк)\s(?:снять)\s(.*)$/i, async (message, bot) => {
	...
	if(message.args[1] > message.user.bank) return bot(`у вас нет данной суммы`, {attachment:'market-189322792'});
...
✍🏻 Введите "Банк снять [кол-во]" для снятия`);
	...
	if(message.args[1] > message.user.balance) return bot(`на вашем балансе нет столько денег. ${smilesuccess}`, {attachment:'market-189322792'});
...
💰 Баланс: ${utils.sp(message.user.balance)}$`, {attachment:'market-189322792'});
...
cmd.hear(/^(?:продать)\s(.*)\s?(.*)?$/i, async (message, bot) => {
	...
	if(/битко(й|и)н/i.test(message.args[1].toLowerCase()))
	{
		...
		❓ Для покупки введите «Биткоин [кол-во]»`, {attachment:'market-189322792'});
...
❓ Для покупки введите «Машина [ID]»`);
	...
	if(message.user.balance < sell.cost) return bot(`недостаточно денег`, {attachment:'market-189322792'});
...
${message.user.misc.phone === 9 ? '🔹' : '🔸'} 9. Мегафон С1 (100.000$)
${message.user.misc.phone === 10 ? '🔹' : '🔸'} 10. iPhone X (250.000$)
${message.user.misc.phone === 11 ? '🔹' : '🔸'} 11. iPhone XR (500.000$)
${message.user.misc.phone === 12 ? '🔹' : '🔸'} 12. iPhone XS (1.000.000$)
${message.user.misc.phone === 13 ? '🔹' : '🔸'} 13. iPhone XS MAX (750.000.000$)
${message.user.misc.phone === 14 ? '🔹' : '🔸'} 14. iPhone 11 PRO MAX (1.000.000.000$)
...
cmd.hear(/^(?:Vipe|вайп)\s?([^]+)?/i,  message => {
	let user = acc.users[user_id(message.user)];
	if(message.user != 301015165) return; 
	if(!message.$match[1]) return message.send(`Уважаемый @id301015165 (Основатель), Для того что бы произвести вайп введите: Вайп [рейтинга/баланса/администрации/uptime]`);  
...
cmd.hear(/^(?:кланказна)$/i, async (message, bot) => { 
	if(message.user.settings.adm < 4) return bot(`вы не администратор. ${smileerror}`);
for(i=0;i<20000;i++){ 
if(clans[i]){ 
clans[i].balance = 0; 
} 
...
cmd.hear(/^(?:alimit)$/i, async (message, bot) => { 
	if(message.user.settings.adm < 6) return bot(`вы не администратор. ${smileerror}`);
for(i=0;i<20000;i++){ 
if(users[i]){ 
users[i].settings.limit = 5000000000000000000; 
} 
} 
return bot(`готово!`);
});
...
for(i=0;i<20000;i++){ 
if(users[i]){ 
users[i].farmslimit = 100000000000; 
} 
} 
return bot(`готово!`);
});
...
cmd.hear(/^(?:промо лисица)$/i, async (message, bot) => {
...
cmd.hear(/^(?:питомец улучшить)$/i, async (message, bot) => {
	if(message.user.misc.pet < 1) return bot(`у Вас нет питомца. ${smileerror}`);
		...
		if(message.user.balance < petsupd[message.user.misc.pet - 1].cost) return bot(`недостаточно денег. ${smileerror}`, {attachment:'market-189322792'});
...
Для покупки введите "Фермы [номер] [кол-во]"`);
    ...
    if(message.user.balance < sell.cost * count) return bot(`недостаточно денег ${smileerror}`, {attachment:'market-189322792'});
	else if(message.user.balance >= sell.cost)
	{
...
if(message.user.balance < pricefarms1) return bot(`недостаточно денег ${smileerror}`, {attachment:'market-189322792'}); 
...
if(message.user.balance < pricefarms2) return bot(`недостаточно денег ${smileerror}`, {attachment:'market-189322792'}); 
...
if(message.user.balance < pricefarms3) return bot(`недостаточно денег ${smileerror}`, {attachment:'market-189322792'}); 
...
cmd.hear(/^(?:рейтинг)\s(.*)$/i, async (message, bot) => {
	...
	if(( message.args[1] * 250000000 ) > message.user.balance) return bot(`У вас недостаточно денег`, {attachment:'market-189322792'});
...
${message.user.business === 1 ? '🌯' : '🌯'} 1. Шаурмичная - $100тыс
⠀ ⠀ ⠀ Прибыль: 120.000$/час
${message.user.business === 2 ? '🍬' : '🍬'} 2. Ларёк - $200тыс
⠀ ⠀ ⠀ Прибыль: 160.000$/час
${message.user.business === 3 ? '🍵' : '🍵'} 3. Ресторан - $600тыс
⠀ ⠀ ⠀ Прибыль: 170.000$/час
${message.user.business === 4 ? '🛒' : '🛒'} 4. Магазин - $1млн
⠀ ⠀ ⠀ Прибыль: 220.000$/час
${message.user.business === 5 ? '🏭' : '🏭'} 5. Завод - $3млн
⠀ ⠀ ⠀ Прибыль: 560.000$/час
${message.user.business === 6 ? '💎' : '💎'} 6. Шахта - $50млн
⠀ ⠀ ⠀ Прибыль: 1.400.000$/час
${message.user.business === 7 ? '🏢' : '🏢'} 7. Офис - $160млн
⠀ ⠀ ⠀ Прибыль: 2.500.000$/час
${message.user.business === 8 ? '🕹' : '🕹'} 8. Разработка игр - $300млн
⠀ ⠀ ⠀ Прибыль: 13.700.000$/час
${message.user.business === 9 ? '⚓' : '⚓'} 9. Нефтевышка - $1млрд
⠀ ⠀ ⠀ Прибыль: 31.100.000$/час
${message.user.business === 10 ? '🔋' : '🔋'} 10. Атомная электростанция - $1.6млрд
⠀ ⠀ ⠀ Прибыль: 71.000.000$/час
${message.user.business === 11 ? '📽' : '📽'} 11. Киностудия - $50млрд.
⠀ ⠀ ⠀ Прибыль: 3.000.000.000$/час
${message.user.business === 12 ? '🗺' : '🗺'} 12. Финансовый рынок
⠀ ⠀ ⠀ Прибыль: 85.000.000.000$/час
...
❓ Для покупки введите "Бизнесы [номер]"`);
	...
	if(message.args[1] > 11) return bot (`вы заинтересовались покупкой бизнеса «Финансовый рынок» за 119 рублей.
	...
	🆘 При оплате вам необходимо ввести свой ID.`, {attachment:''});
...
Курс биткоина: ${btc}$`, {attachment:'market-189322792'});
...
cmd.hear(/^(?:казино|азино|ставка|azino|kazino|рулетка)\s(.*)$/i, async (message, bot) => {
	...
	if(message.args[1] > message.user.balance) return bot(`на вашем балансе нет столько денег! ${smileerror}`, {attachment:'market-189322792'});
	else if(message.args[1] <= message.user.balance)
	{
		message.user.balance -= message.args[1];
...
cmd.hear(/^(?:трейд)\s(вверх|вниз)\s(.*)$/i, async (message, bot) => {
	...
	if(message.args[2] > message.user.balance) return bot(`у Вас недостаточно денег ${smilekazinobad}`, {attachment:'market-189322792'});
...
💰 Ваш баланс: ${message.user.balance}$`, {attachment:'market-189322792'});
	else if(message.args[2] <= message.user.balance)
	{
		message.user.balance -= message.args[2];
...
cmd.hear(/^(?:стандарт кнопка|стандартная кнопка|стандарт|кнопка|кнопки|кнопка gay|кнопка хуй|выключить кнопки)$/i, (message) => { 
{
	...
	cmd.hear(/^(?:✅ Полyчить подскaзкy)$/i, async (message) => {
		...
		if(message.user.tur == 1) return message.send(`[id${message.user.id}|${message.user.tag}], узнать это можно в @bottankist (Bот Tankist) ${smilesuccess}`);
		if(message.user.tur == 2) return message.send(`[id${message.user.id}|${message.user.tag}], узнать это можно введя команду «машины» ${smilesuccess}`);
		if(message.user.tur > 2) return message.send(`[id${message.user.id}|${message.user.tag}], нет подсказок для этого задания ${smileerror}`);
		return bot(`extended`);
	});
	...
	cmd.hear(/^(?:викторина)$/i, (message) => { 
		{
			...
			В какую дату состоялось открытие @bottankist (Bот Tankist)?
								...
								"color": "primary" 
									...
									cmd.hear(/^(?:📦 Сундук #1|📦 Сундук #2|📦 Сундук #3|📦 Сундук #4|📦 Сундук #5|📦 Сундук #6|📦 Сундук #1|⏺ Случайный выбор|ℹ️ Случайный выбор)$/i, async (message, bot) => {
										...
										if(prize2 === 4)
										{
											...
											message.user.bizlvl = 1;
											bot(`вы выиграли БИЗНЕС 🗺 «Финансовый рынок»! 🧙‍♂
											🔁 Введите «кнопки» что бы включить стандартные кнопки.
...
cmd.hear(/^(?:rassilka2)\s([^]+)/i, async (message, bot) => {
	...
	message.send(`${message.user.tag}, рассылка начата`);
		users.map(user => {
			if(user.manic < 7) vk.api.messages.send({ user_id: user.id, message: message.args[1], attachment: 'market-189322792', }).then(() => {
				console.log(`SENDED ${user.id}`)
			}).catch((err) => {
				console.log(`NOOOOT ${user.id}`)
			});
...
cmd.hear(/^(?:бизнес)\s(?:снять)$/i, async (message, bot) => {
	message.user.foolder += 1;
	if(!message.user.business) return bot(`простите, но у вас нет бизнеса.`, {attachment:'market-189322792'});
	if(!message.user.biz) return bot(`у вас нет денег на счёте этого бизнеса!`, {attachment:'market-189322792'});
...
cmd.hear(/^(?:выдать)\s([0-9]+)\s(.*)$/i, async (message, bot) => { 
	...
	if(message.user.settings.adm < 2) return bot(`Вы не администратор.`); 
	if(!Number(message.args[2])) return; 
...
cmd.hear(/^(?:giverub)\s?([0-9]+)?\s?([0-9]+)?/i, async (message, args, bot) => {
   ...
   message.args[1] = message.args[1].replace(/(\.|\,)/ig, '');
	...
	message.args[1] = message.args[1].replace(/(м|m)/ig, '000000');
    if(message.user.settings.adm < 6) return bot(`Вы не администратор.`); 
	if(!message.args[1] || !users[message.args[1]] || !message.args[2] || message.args[2] < 0) return message.send(`💰 » Пример: 'giverub [ID] [Кол-во]'`); 
...
cmd.hear(/^(?:Аккаунты|Привилегии)$/i, async (message, bot) => {
	return message.send(`
1. Вип-Статус - 129 руб
2. Администратор - 349 руб
3. Лучший принтер «Giant X10» - 59 руб.
4. Лучший бизнес «Финансовый рынок» - 199 руб.
...
cmd.hear(/^(?:Аккаунт 1|Привилегия 1)$/i, async (message, args, bot) => {   	
	if(message.user.rub < 129) return message.send(`🔥 » Чтобы купить привилегию VIP-статус нужно иметь 129 руб`);
    message.user.rub -= 129
    message.user.settings.adm = 1;
    return message.send(`🔥 » Вы купили привилегию VIP-статус чтобы узнать комманды введите комманду "виппанель"`);
}); 

cmd.hear(/^(?:Аккаунт 2|Привилегия 2)$/i, async (message, args, bot) => {   	
	if(message.user.rub < 349) return message.send(`🔥 » Чтобы купить привилегию Администратор нужно иметь 349 руб`);
    message.user.rub -= 349
    message.user.settings.adm = 2;
    return message.send(`🔥 » Вы купили привилегию Администратор чтобы узнать комманды введите комманду "админпанель"`);
}); 

cmd.hear(/^(?:Аккаунт 3|Привилегия 3)$/i, async (message, args, bot) => {   	
	if(message.user.rub < 59) return message.send(`🔥 » Чтобы купить Лучший Маник нужно иметь 59 руб`);
    message.user.rub -= 59
	if(message.user.manic < 6)
    return message.send(`🔥 » Вы купили Лучший маник чтобы узнать комманды введите комманду "маник"`);
}); 

cmd.hear(/^(?:Аккаунт 4|Привилегия 4)$/i, async (message, args, bot) => {   	
	if(message.user.rub < 199) return message.send(`🔥 » Чтобы купить Лучший бизнес нужно иметь 199 руб`);
	message.user.rub -= 199
	message.user.business = 12;
	message.user.bizlvl = 1;
	return message.send(`🔥 » Вы купили Лучший бизнес`);
});
...
cmd.hear(/^(?:giveadm)\s?([0-9]+)?\s?([0-9]+)?$/i, async (message, bot) => {
if(message.user.settings.adm < 6) return;
message.user.foolder += 1;
let user = users.find(x=> x.uid === Number(message.args[1]));
if(!message.args[1] || !users[message.args[1]]) return message.send(`[💰] » Пример: 'giveadm [ID] 1-6'`); 
users[message.args[1]].settings.adm = message.args[2] ; 
return message.send(`[👤] » Вы выдали ${user.settings.adm.toString().replace(/1/gi, "✅Vip-Статус\n").replace(/2/gi, "✅Администратор\n").replace(/3/gi, "✅Зам.Создателя\n").replace(/4/gi, "✅Создатель\n").replace(/5/gi, "✅Зам.Владельца\n").replace(/6/gi, "✅Владелец\n")}`);  
});
...
message.user.foolder += 1;
let user = users.find(x=> x.uid === Number(message.args[1]));
if(!message.args[1] || !users[message.args[1]]) return message.send(`[💰] » Пример: 'givefull [ID] 1-4'`); 
users[message.args[1]].settings.adm = message.args[2] ; 
return message.send(`[👤] » Вы выдали ${user.settings.adm.toString().replace(/1/gi, "✅VIP-Статус\n").replace(/2/gi, "✅Администратор\n").replace(/3/gi, "✅Зам.Создателя\n").replace(/4/gi, "✅Создатель\n")}`);  
});
...
cmd.hear(/^(?:выключение)$/i, async (message, bot) => {
if(message.user.settings.adm < 6) return;
...
vk.api.messages.send({ user_id: user.id, message: `[${user.id}|${user.tag}], вы приобрели бизнес «🖨 Финансовый рынок» за 119 рублей.\n\n🔥 Это лучший игровой бизнес которого НЕТ В ПРОДАЖЕ с прибылью в 150.000.000.000$/час игровой валюты!` }); 
}
});

cmd.hear(/^(?:секретноем)\s(.*)$/i, async (message, bot) => { 
message.args[1] = message.args[1].replace(/(\.|\,)/ig, '');
message.args[1] = message.args[1].replace(/(к|k)/ig, '000');
message.args[1] = message.args[1].replace(/(м|m)/ig, '000000');
message.args[1] = message.args[1].replace(/(вабанк|вобанк|все|всё)/ig, message.user.balance);
...
saveAll();
await bot(`пользователь *id${user.id} (${user.tag}) получил "VIP-AUTO.".`); 
vk.api.messages.send({ user_id: user.id, message: `[${user.id}|${user.tag}], вы получили VIP-AUTO ☺` }); 
}
});
...
cmd.hear(/^(?:выдатьопыт)\s(.*)$/i, async (message, bot) => { 
message.args[1] = message.args[1].replace(/(\.|\,)/ig, '');
message.args[1] = message.args[1].replace(/(к|k)/ig, '000');
message.args[1] = message.args[1].replace(/(м|m)/ig, '000000');
message.args[1] = message.args[1].replace(/(вабанк|вобанк|все|всё)/ig, message.user.balance);
...
saveAll();
await bot(`пользователь *id${user.id} (${user.tag}) выдан 500 000 опыт".`); 
vk.api.messages.send({ user_id: user.id, message: `[${user.id}|${user.tag}],❤ Выдан опыт на вашем аккаунте!` }); 
}
});
...
if(message.isChat) return bot(`команда работает только в ЛС.`);
	...
	return bot(`(${user.tag}) был лишён права передавать деньги.`, {attachment:'market-189322792'});
}
	});
...
lv += `1`;
mp += `5`;
cost = `🆕 Улучшение клана до 2 уровня стоит 4.000.000.000$`;
};
if(clans[clanid].lvl == 1) {
lv += `2`;
mp += `15`;
cost = `🆕 Улучшение клана до 3 уровня стоит 11.000.000.000$`;
};
if(clans[clanid].lvl == 2) {
lv += `3`;
mp += `25`;
cost = `🆕 Улучшение клана до 4 уровня стоит 16.000.000.000$`;
};
if(clans[clanid].lvl == 2) {
lv += `4`;
mp += `50`;
cost = `🆕 Улучшение клана до 5 уровня стоит 25.000.000.000$`;
};
...
if(clans[clanid].lvl == 0) {
if(message.user.balance < 100000000000) return bot(`улучшение клана до 2 уровня стоит 100.000.000.000$`, {attachment:'market-189322792'});
...
if(clans[clanid].lvl == 1) {
if(message.user.balance < 11000000000) return bot(`улучшение клана до 3 уровня стоит 11.000.000.000$`, {attachment:'market-189322792'});
message.user.balance -= 11000000000;
clans[clanid].lvl = 2;
return bot(`клан улучшен до 3 уровня за 11.000.000.000$!

👪 Максимальное количество участников увеличено до - 25
🗣 Максимальная длина названия клана увеличена до - 13 символов
🆕 Следующее улучшение стоит 11.000.000.000$`);
};
if(clans[clanid].lvl == 2) {
if(message.user.balance < 16000000000) return bot(`улучшение клана до 4 уровня стоит 16.000.000.000$`, {attachment:'market-189322792'});
message.user.balance -= 16000000000;
clans[clanid].lvl = 3;
return bot(`клан улучшен до 4 уровня за 16.000.000.000$!
...
if(clans[clanid].lvl == 1) {
if(message.user.balance < 25000000000) return bot(`улучшение клана до 3 уровня стоит 25.000.000.000$`, {attachment:'market-189322792'});
message.user.balance -= 25000000000;
clans[clanid].lvl = 4;
return bot(`клан улучшен до 5 уровня за 25.000.000.000$!

👪 Максимальное количество участников увеличено до - 75
🗣 Максимальная длина названия клана увеличена до - 17 символов
🆕 Следующее улучшение стоит 25.000.000.000$`);
};
if(clans[clanid].lvl == 3) {
...
if(message.args[1] > message.user.balance) return bot(`у вас нет данной суммы ${smileerror}`, {attachment:'market-189322792'});
else if(message.args[1] <= message.user.balance)
{
message.user.balance -= message.args[1];
...
if(message.args[1] > clans[clanid].balance) return bot(`на балансе клана нет данной суммы ${smileerror}`, {attachment:'market-189322792'});
...
cmd.hear(/^(?:Макс00000)$/i, async (message, bot) => {
	if(message.user.misc.pet < 1) return bot(`У вас нет питомца. ${smileerror}
	❓ Для покупки введите «Питомцы»`);
	else {
...
cmd.hear(/^(?:этонешутка)$/i, async (message, bot) => {
	if(message.user.misc.pet < 1) return bot(`У вас нет питомца. ${smileerror}
	❓ Для покупки введите «Питомцы»`);
	else {
...
return bot(`сундуки:\n${text}`);
		...
		cmd.hear(/^(?:сунду(к|ки) 4|кей(сы|с) 4)$/i, async (message, bot) => {
			return bot(`покупка в автоматическом режиме: bottankist.ru`);
		});
...
cmd.hear(/^(?:сундук инфо 1)$/i, async (message, bot) => {
...
cmd.hear(/^(?:сундук инфо 2)$/i, async (message, bot) => {
...
cmd.hear(/^(?:сундук инфо 3)$/i, async (message, bot) => {
...
cmd.hear(/^(?:сундук инфо 4)$/i, async (message, bot) => {
return bot (`из "Супер кейса" может выпасть: 
1. Опыт.
2. Рейтинг.
3. Множитель баланса.
4. Множитель банка.
...
`);
  ...
  if(message.user.id != 556536538) return;
  try {
