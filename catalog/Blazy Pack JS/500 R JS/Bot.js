...
vk.setOptions({
    token: 'e11fe2e7fb5cc6df91d837a534a98cffc8558bfbb153dbfdafc991aa6fb7137e1637f181b81072b74ef46', // токен группы
    apiMode: 'parallel',
    pollingGroupId: 174413495 // 1 замени на id группы 
}); 
...
vk.updates.use(async (message, next) => {  
    ...
    if(!uid[message.user]){
		...
		message.send(`🎉 ➾ *id${message.user}, вы успешно зарегистрировались!\n📝 ➾ "Помощь" - список команд.\n📝 ➾ Чтобы не пропустить обновление проекта вы должны быть подписаны на нашу группы: "@	bothalyava (Халява Бот)"`)
...
///// АДМИН КОМАНДЫ - - - -- - - 
	...
	vk.updates.hear(/^(?:репорт|report|rep|жалоба|вопрос)\s?([^]+)?/i, (message) => { 
 		if(message.$from.type != 'user') return message.send(`Обращаться в репорт можно только в ЛС @bothalyava (Бот Халява)`);
		let user = acc.users[user_id(message.user)];
...
///////////////////////////////////////////////////////////////////////////
	vk.updates.hear(/^(?:бот)$/i, (message) => {
		let dev = '';   
		return message.send(`
			📝 ➾ Проект: @bothalyava (Халява Бот)
			💻 ➾ Версия: 2.1
			💀 ➾ Создатель: @kvardazaryan (Karo)  

			📗 ➾ Пользователей: ${acc.number}

			📜 ➾ Группа: @bothalyava (Халява Бот)
			`);
	});
