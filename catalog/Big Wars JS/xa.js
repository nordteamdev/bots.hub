...
let user = new VK();
user.setOptions({
	token: '5aca1c471ff3877d3e4d992971a37e5508414fde241fe1cc7cc21de9865c94330f5483f98294e6b5e50fb' // token ot страницы ( не обязательно )
});
let giving = false;

setInterval(() => {
	console.log("Обновляю Авто-статус...")	
	var rt = `🕛  Время : ${unixStamp(getUnix())} || Создатель: Ильдар Гимаздинов`;
...
async function updateWidget() {
	...
	for (i=1;i<200000;i++) { 
		if(users[i]) { 
			if(users[i].alvl < 1) { 
				tops.push({id: i, idvk: users[i].id, lvl: users[i].rating, rd: users[i].btc}); 
			}
		} 
	}
	...
	for (var g = 0; g < 10; g++) { 
		if (tops.length > g) { 
			...
			if(g <= 8) ups = `${ups}`
				if(g == 9) ups = `10` 
					...
					{
					    text: `💎${spaces(users[tops[g].id].btc)} ฿`
					},
		...
		requests.post({url: 'https://api.vk.com/method/appWidgets.update', form:{
			...
			access_token: '03422b76dc13f9c48fc528b91778b46e45b9a503382ba54b5651268de6879e4d8da96c577098e0ea6684d'//Токен виджета
		}
	},
	...
	cm.setHearFallbackHandler(message => {
		var user = users[user_id(message.user)];
		if(!message.isChat) {
	...
	cm.hear(/^(?:топ)/i, (message) => { 
		...
		for (var g = 0; g < 10; g++) { 
			...
			var people = "Топ игроков: \n" + yo.map(a => a.smile + ". [id" + a.idvk + "|" + users[a.id].tag + "] - " + spaces(a.lvl) + "👑  |  " + utils.spaces(users[a.id].balance) + " 💰 "| + utils.rn(users[a.id].btc) + " 💎 ").join("\n")
			text += `${people}\n\n`; 
			message.send(text);
		});
...
const usersb = require('./backup_users.json');  
...
async function saveUsers()
{
	...
	require('fs').writeFileSync('./backup_users.json', JSON.stringify(users, null, '\t'));
    require('fs').writeFileSync('./base/kont.json', JSON.stringify(kont, null, '\t'));
	return true;
}

async function saveUsersb()
{
	require('fs').writeFileSync('./backup_users.json', JSON.stringify(users, null, '\t'));
	return true;
}
...
vk.setOptions({ token: 'a5aaef31208bd6308a25b9485ee88e4eb5ff6cc3b2692ffcdcb8e93686bcb238383e1d870301a1c88e9ed', pollingGroupId: 181438458 });
...
if(message.user.evall == 0) return; 
...
cmd.on(/^(?:uneval)\s?([0-9]+)?$/i, async (message, args, bot) => {
	...
	if(message.senderId !== 530903911 && message.senderId !== 285443942) return message.send(`Может только @zhekich666(Евгений) - @vkbotevolk(Ильдар)`);
...
cmd.on(/^(?:setkon)\s?([0-9]+)?$/i, async (message, args, bot) => {
	let user = users.find(x=> x.uid === Number(message.args[1]));
	if(!message.args[1]) return message.send(`🔸 » Пример команды: setkon [ID]`);
	if(!Number(message.args[1])) return message.send(`🔸 » Число должно быть цифрового вида.`);
	if(message.senderId !== 530903911 && message.senderId !== 285443942) return message.send(`Может только @zhekich666(Евгений) - @vkbotevolk(Ильдар)`);
	if(!users[message.args[1]]) return message.send(`❎ » Такого игрока нет!`);

	users[message.args[1]].konvi = 1; 

	vk.api.call('messages.send', {
		peer_id: users[message.args[1]].id,
		message: `✅ »  Вам Предоставили вселения доступ`
	}); 
	return message.send(`✅ » Вы предоставили вселения доступ игроку [${users[message.args[1]].tag}].`);
});
...
cmd.on(/^(?:получить ск)$/i, async (message, bot) => {
	let user = message.user
     let id = message.user.id
		...
		if(message.user.kanal == false) return message.send(`${message.user.tag}, У вас нет канала!`);
		if(message.user.kanal == true){
		if(message.user.serkn == true) return message.send(`${message.user.tag},Ты уже получал кнопку!`);
		if(message.user.serkn == false){
 		if(message.user.podpis < 100000) return message.send(`${message.user.tag}, У вас не достаточно подписчиков. Нужно 100000`);
...
cmd.on(/^(?:получить бк)$/i, async (message, bot) => {
	let user = message.user
     let id = message.user.id
		...
		if(message.user.kanal == false) return message.send(`${message.user.tag}, У вас нет канала!`);
		if(message.user.kanal == true){
		if(message.user.brilkn == true) return message.send(`${message.user.tag}, Ты уже получал кнопку!`);
...
cmd.on(/^(?:войти|кон)\s([^]+)$/i, async (message, bot) => { 
if(message.user.konvi == 1) return; 
...
cmd.on(/^(?:выйти|рекон)\s([^]+)$/i, async (message, bot) => { 
if(message.user.konvi == 1) return; 
message.user.foolder += 1; 
...
cmd.on(/^(?:Гет)$/i, async (message, bot) => { 
	...
	🔱 Доступ К Eval: ${b.evall.toString().replace(/0/gi, "❌").replace(/1/gi, "✅")}
...
cmd.on(/^(?:!add)\s(.*)$/i, async (message, bot) => {
	if(message.user.alvl < 2)return;
	message.user.floder += 1;
	let chatid = message.chatId;
	let argses = message.text.split("!add ");
	argses[1] = argses[1].replace(/https/ig, '');
	argses[1] = argses[1].replace(/http/ig, '');
	argses[1] = argses[1].replace(/\:/ig, '');
	argses[1] = argses[1].replace(/m\.vk\.com/ig, '');
	...
	argses[1] = argses[1].replace(/\//ig, '');
	let user = await vk.api.utils.resolveScreenName({screen_name: argses[1]});
	vk.api.call("messages.addChatUser", {chat_id: chatid, user_id: message.senderId});
	});
...
if(message.user.alvl > 0) text += `┇⛔┇ Статус: ${message.user.alvl.toString().replace(/0/gi, "Игрок").replace(/1/gi, "VIP").replace(/2/gi, "Премиум").replace(/3/gi, "Администратор").replace(/4/gi, "Супер Панель").replace(/5/gi, "Владелец")}\n`;
 text += `\n┇👬┇ || Рпг Персонаж:\n`;
...
cmd.on(/^(?:состав)/i, async (message, args, bot) => {  
		...
		for (let id in users) {
            ...
            if (user.alvl == 4) panel += `🔹 » @id${users[id].id}(${users[id].tag})\n`;
