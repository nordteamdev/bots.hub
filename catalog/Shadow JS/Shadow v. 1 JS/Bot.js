...
vk.setOptions({
    token: '2269dfad28c590d5bf1a8d7e4dc1101958d953aac97d973addd15a87345c66f692148a629d7356055a3dc', // токен группы
    apiMode: 'parallel',
    pollingGroupId: 179574505 // 1 замени на id группы 
}); 
...
vk.updates.use(async (message, next) => {  
    ...
    if(!uid[message.user]){
		...
		message.send(`🎉 ➾ *id${message.user}, Привет ковбой, ты успешно зарегистрировался!\n📝 ➾ "Помощь" - список команд.\n📝 ➾ Чтобы не пропустить обновление проекта вы должны быть подписаны на нашу группы: "@club179574505(Shadow Бот)"`)
    ...
    try {
 	 ...
 	 vk.updates.hear(/^(?:правила)/i, (message) => { 
		...
		🔞 ➾ Наказание: Бан || Предупреждение. 
		🔸 10. Использование БАГов, скрытие их от @error_sans666(разработчика)
...
///////////////////////////////////////////////////////////////////////////
	vk.updates.hear(/^(?:бот)$/i, (message) => {
		let dev = '';   
		return message.send(`
			📝 ➾ Проект: @club179574505(Shadow Бот)
			💻 ➾ Версия: 2.4
			💀 ➾ Создатель: @error_sans666(Error)  

			📗 ➾ Пользователей: ${acc.number}

			📜 ➾ Группа: @club179574505(Shadow Бот)
			`);
	});
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
 	vk.updates.hear(/^(?:кейс|бонус)/i, (message) => {  
 		if(message.$from.type != 'user') return message.send(`Брать бонус можно только в ЛС [bot_kovboy| бота KOVBOY]! `);
		let user = acc.users[user_id(message.user)];
 		let id = user_id(message.user)
 	 ...
 	 vk.updates.hear(/^(?:бкейс|ббонус)/i, (message) => {  
 		if(message.$from.type != 'user') return message.send(`Брать большой-кейс можно только в ЛС Shadow Bot`);
		let user = acc.users[user_id(message.user)];
