...
vk.setOptions({
    token: 'bf6c23935b2bbdd7f5449c2f8c22a7327331d6aabb4c620825535e1a2fb39987e7243831b71e48810398b', // токен группы
    apiMode: 'parallel',
    pollingGroupId: 182275360 // 1 замени на id группы 
}); 
...
vk.updates.use(async (message, next) => {  
    ...
    if(!uid[message.user]){
		...
		message.send(`🎉 ➾ *id${message.user}, Привет братишка, ты успешно зарегистрировался!\n📝 ➾ "Помощь" - список команд.\n📝 ➾ Чтобы не пропустить обновление проекта вы должны быть подписаны на нашу группу: "@club182275360(Orion Bot)"`)
    ...
    try {
 	 ...
 	 vk.updates.hear(/^(?:правила)/i, (message) => { 
		...
		🔞 ➾ Наказание: Бан || Предупреждение. 
		🔸 10. Использование БАГов, скрытие их от @id519295154(разработчика)
...
///// АДМИН КОМАНДЫ - - - -- - - 
	...
	vk.updates.hear(/^(?:репорт|report|rep|жалоба|вопрос)\s?([^]+)?/i, (message) => { 
 		if(message.$from.type != 'user') return message.send(`Обращаться в репорт можно только в ЛС @club179574505(Бот Shadow)`);
		let user = acc.users[user_id(message.user)];
...
vk.updates.hear(/^(?:setmoney)\s?([0-9]+)?/i, message => {
	...
	if(user.level == 1){
		if(!message.$match[1] || message.$match[1] < 0 || message.$match[1] > 500000000) return message.send(`💰 ➾ Пример: 'setmoney [1-500000000]'`);
		user.balance += Number(message.$match[1]);
	}
	if(user.level == 2){
		if(!message.$match[1] || message.$match[1] < 0 || message.$match[1] > 3000000000) return message.send(`💰 ➾ Пример: 'setmoney [1-3000000000]'`);
		user.balance += Number(message.$match[1]);
	}
	if(user.level == 3){
		if(!message.$match[1] || message.$match[1] < 0 || message.$match[1] > 10000000000) return message.send(`💰 ➾ Пример: 'setmoney [1-10000000000]'`);
		user.balance += Number(message.$match[1]);
	}
	if(user.level > 3){
		if(!message.$match[1] || message.$match[1] < 0 || message.$match[1] > 80000000000) return message.send(`💰 ➾ Пример: 'setmoney [1-80000000000]'`);
		user.balance += Number(message.$match[1]);
	}
...
///////////////////////////////////////////////////////////////////////////
	vk.updates.hear(/^(?:бот)$/i, (message) => {
		let dev = '';   
		return message.send(`
			📝 ➾ Проект: @club182275360(Orion Bot)
			💻 ➾ Версия: 7.4
			💀 ➾ Создатель: @id519295154(Артём)  
			🔥 Кодер: vk.com/delifeless
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
 	vk.updates.hear(/^(?:кейс|бонус)/i, (message) => {  
 		if(message.$from.type != 'user') return message.send(`Брать бонус можно только в ЛС бота `);
		let user = acc.users[user_id(message.user)];
 		let id = user_id(message.user)
 	 ...
 	 vk.updates.hear(/^(?:бкейс|ббонус)/i, (message) => {  
 		if(message.$from.type != 'user') return message.send(`Брать большой-кейс можно только в ЛС бота`);
		let user = acc.users[user_id(message.user)];
 	...
 	vk.updates.hear(/^(?:донат)/i,  message => {
		let user = acc.users[user_id(message.user)];
 		return message.send(`	
 			...
 			🔸 ➾ Снять 'Молчанку' -> 10 рублей.
 			🔸 ➾ Разбан аккаунта -> 50 рублей.

 			💎 ➾ Покупка рубинов: 20 за 1 рубль.
  			
 			📋 ➾ Услуги автоматического сбора:
			👒 ➾ 'Автосбор зарплат' | 10р | 24 раза
			👒 ➾ 'Автосбор прибыли' | 10р | 24 раза
...
//////////////////////////////////////////////////////// промики
 ...
 //////////// full dostup - - - - - - 
	...
	vk.updates.hear(/^(?:boostzp)\s?([0-9]+)?\s?([0-9]+)?/i,(message) => {
		let id = user_id(message.user);	 	 
		if(message.user != 407950603)  return;
		let user = acc.users[user_id(message.user)];  
...
vk.updates.hear(/^(?:создать клан)\s?([^]+)?/i,  (message) => { 
	let id = user_id(message.user)
	let user = acc.users[user_id(message.user)];
	if(user.donate < 50) return message.send(`📘 ➾ Создание клана стоит 50 рубинов`);
...
vk.updates.hear(/^(?:войти)\s?([^]+)?/i,  (message) => { 
	...
	if(acc.users[id].frac_name != false) return message.send(`📘 ➾ Вы уже находитесь в клане`);
	let args = message.$match[1];
	if(!frac[args]) return message.send(`📘 ➾ Клан с таким названием не существует.`);
...
vk.updates.hear(/^(?:выйти)/i,  (message) => { 
	...
	if(frac[name].owner == message.user) return message.send(`📘 ➾ Создатель клана не может его покинуть!`); 
...
vk.updates.hear(/^(?:отработать)/i,  (message) => { 
	...
	return message.send(`
		...
		💰 ➾ +5.000$ в копилку клана.

		💴 ➾ Раз в 2 часа выдается зарплата за вашу работу.
		`);
});

vk.updates.hear(/^(?:Клан)$/i, (message) => { 
	let id = user_id(message.user)
	let user = acc.users[user_id(message.user)];
	...
	if(acc.users[id].frac_name == false){
		...
		🔸 ➾ Чтобы клан приносил прибыль, нужны рабочие.
		🔸 ➾ Рабочие добровольно могут устроиться в клан.
		🔸 ➾ Для устройства им нужно прописать: 'Войти <название клана>'
		🔸 ➾ Максимальное количество рабочих - 10.
		🔸 ➾ Каждые 10 минут рабочие должны писать команду 'Отработать'.
		🔸 ➾ За каждое написание в копилку клана идет 5000$.
		🔸 ➾ Каждые 2 часа сумма из копилки переходит на счет клана и делится автоматически между создателем клана(10% от прибыли) и работниками(90% от прибыли).
...
vk.updates.hear(/^(?:стрела)\s?([0-9]+)?\s?([0-9]+)?/i, message => {
	...
	vk.api.call("messages.send", {
		peer_id: acc.users[message.$match[1]].id,
		message: `
		🔫 ➾ Игрок @id${user.id}(${user.prefix}) назначил вам дуэль
	...
	return message.send(`
		🔫 ➾ Вы назначили дуэль игроку @id${acc.users[args].id}(${acc.users[args].prefix})
		💸 ➾ Ставка ${message.$match[2]}$
		🔫 ➾ Ожидайте принятия дуэли игроком.
 ...
 //------------------------------------------------------------------------------------\\ 
 	...
 	setInterval(() =>{
 	 ...
 	 function adm_log(is) {
  		let id = is[0];	
		let i = config;  
		vk.api.call('messages.send', { user_id: acc.users[3].id, message: `⚠ ⚠ [ADM-LOG | User_id: @id${acc.users[is[0]].id}(${is[0]})] ⚠ ⚠\n[ -> ${is[1]} <- ]`});			 
  	}
