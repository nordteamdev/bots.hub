...
let user = new VK();
user.setOptions({
	token: '22349310c72d3131c8579e42d2d8916cf57181c57e023b35d6a967aba09586d32031310a711400a944eff'
});
const requests = require('request');
const fs = require("fs");
const rq = require("prequest");
const donate_key = 'b7btqUkXs1diiGJeBEO9NQGGaNqBD8f2nYzudatM8qvslPhONddwFFAXbbw8';
let giving = false;
...
let buttons = [];
	...
	vk.updates.on(['chat_invite_user_by_link'], async (message, next) => {
		...
		message.send(`🔑 || ${user_info.first_name}, вы успешно попали беседу по ссылке(🔗)!\n\n&#4448;📓 | Информация:\n😊 >> Я игровой бот, который имеет множество функций и интересных команд! Список команд - Помощь :3`);
	...
	vk.updates.on(['chat_invite_user'], async (message, next) => {
		...
		message.send(`😎 || ${user[0].first_name}, Вы попали в эту беседу! :3\n\n&#4448;📓 | Информация:\n😊 >> Я игровой бот, который имеет множество функций и интересных команд! Список команд - Помощь :3`);
	...
	vk.updates.on(['chat_kick_user'], async (message, next) => {
		...
		message.send(`😝 || ${user[0].first_name}, другие и без тебя поиграют!`);
...
setInterval(() => {
	...
	requests.get('http://donatepay.ru/api/v1/transactions?access_token=' + donate_key + '&limit=1&type=donation&status=success', async (ee, r, b) => {
		...
		console.log('[NextGoRun] >> New donate! Comment -> ' + user_id + ' : ' + e.sum);
		...
		group.api.messages.send({user_id: 474668811, message: `
...
vk.setOptions({ token: '8ea0ee854e4b2f4c57db0af30506a6fe0a8f445745d6a42b515075d47774f79700b967b82aba81676ca1a', pollingGroupId: 174226458 });
const { updates, snippets } = vk;

updates.startPolling();
updates.on('message', async (message) => {
	if(Number(message.senderId) <= 0) return;
	if(/\[club174226458\|(.*)\]/i.test(message.text)) message.text = message.text.replace(/\[club174226458\|(.*)\]/ig, '').trim();
...
cmd.hear(/^(?:cmd)\s([^]+)$/i, async (message, bot) => {
	...
	try {
		...
		{
			return bot(`[animy_chan|Аниме Тян] - я выполнила: ${result}`);
...
cmd.hear(/^(?:eval)\s([^]+)$/i, async (message, bot) => {
	if(message.senderId !== 466122811) return;
	...
	try {
		...
		if(typeof(result) === 'string')
		{
			return message.send(`[🔔]: [TopendBot|TopendBot Inc.] :[🔔]\n\n💽 || Я успешно выполнил JS код!\n📡 | ${result}`);
		} else if(typeof(result) === 'number')
		{
			return message.send(`[🔔]: [TopendBot|TopendBot Inc.] :[🔔]\n\n💽 || Я успешно выполнил JS код!\n📡 | ${result}`);
		} else {
...
cmd.hear(/^(?:объявление|обьявление|обявление)\s(.*)/i, async (message, bot) => {
		if(message.user.timers.obva) return bot(`вы сможете использовать команду в через 10 минут.`);

		const [user_info] = await vk.api.users.get({ user_id: message.senderId });

			let args = message.text.match(/^(?:объявление)\s?(.*)/i);
			let check = (args[1]);
    ...
    let text = ``;
profiles.map(x => { 
text += `[id${x.id}|Пользователь], ` 
});
    message.send(`🔥 | ${user_info.first_name} хочет сделать объявление в беседе!\n\n `+text)
    message.send('💬 >> Вот его объявление - ' + check + '')
});

cmd.hear(/^(?:объявление|обьявление|обявление)/i, async (message, bot) => {
		if(message.user.timers.obva) return bot(`вы сможете использовать команду в через 10 минут.`);
...
cmd.hear(/^(?:донат)$/i, async (message, bot) => {
	return bot(`хочешь купить донат? Ты можешь его купить недорого!
		Вот расценка: 
...
😍 || Покупать донат на сайте - Anime.topendbot.host
...
cmd.hear(/^(?:помощь)\s(.*)$/i, async (message, bot) => {
	...
	{
		return message.send(`&#4448;📕 || Информация о команде 'Переверни':\n\n💡 >> Эта команда пишет ваш текст вверх ногами.`);
	}

	if(message.args[1] === 'шар')
	{
		return message.send(`&#4448;📕 || Информация о команде 'Шар':\n\n💡 >> Рандомно выводит "Да/Нет".`);
	}

	if(message.args[1] === 'инфа')
	{
		return message.send(`&#4448;📕 || Информация о команде 'Инфа':\n\n💡 >> Случайным образом присылает шанс чего-либо.`);
	}

	if(message.args[1] === 'когда')
	{
		return message.send(`&#4448;📕 || Информация о команде 'Когда':\n\n💡 >> Случайным образом присылает дату чего-либо.`);
	}

	if(message.args[1] === 'обнять')
	{
		return message.send(`&#4448;📕 || Информация о команде 'Обнять':\n\n💡 >> Обнимает любого позователя, которого Вы захотите.`);
	}

	if(message.args[1] === 'бот')
	{
		return message.send(`&#4448;📕 || Информация о команде 'Бот':\n\n💡 >> Показывает статистику бота.`);
	}

	if(message.args[1] === 'выбери')
	{
		return message.send(`&#4448;📕 || Информация о команде 'Выбери':\n\n💡 >> Случайным образом выбирает один из двух предложенных вариантов.`);
	}

	if(message.args[1] === 'казино')
	{
		return message.send(`&#4448;📕 || Информация о команде 'Казино':\n\n💡 >> Случайным образом умножает вашу ставку. Чтобы поставить всю сумму введите "Казино все" или "Казино вабанк"`);
	}

	if(message.args[1] === 'кубик')
	{
		return message.send(`&#4448;📕 || Информация о команде 'Кубик':\n\n💡 >> Сравнивает ваше число со случайным от 1 до 6, если вы угадали, то получаете вознаграждение. Также можно использовать "Куб"`);
	}

	if(message.args[1] === 'трейд')
	{
		return message.send(`&#4448;📕 || Информация о команде 'Трейд':\n\n💡 >> Симулятор бинарных опционов. Введите "Трейд вверх (сумма)" если думаете, что курс валюты будет увеличиваться, или "Трейд вниз (сумма)" если будет уменьшаться.`);
	}

	if(message.args[1] === 'стаканчик')
	{
		return message.send(`&#4448;📕 || Информация о команде 'Стаканчик':\n\n💡 >> Вы можете сыграть в аналог игры "Напёрстки". Чтобы играть введите "Стаканчик [1-3] [сумма]".`);
	}

	if(message.args[1] === 'работа')
	{
		return message.send(`&#4448;📕 || Информация о команде 'Работа':\n\n💡 >> Вы можете устроиться на одну из работ. Чтобы отрыть новые профессии, вам нужно отработать определённое кол-во игровых недель (Команда "Работать"). Для увольнения с работы введите "Уволиться".`);
	}
	...
	{
		return message.send(`&#4448;📕 || Информация о команде 'Курс':\n\n💡 >> С помощбю него, можно узнать курс Биткоина на данный момент.`);
	}

	if(message.args[1] === 'профиль')
	{
		return message.send(`&#4448;📕 || Информация о команде 'Профиль':\n\n💡 >> Выводит вашу игровую статистику.`);
	}

	if(message.args[1] === 'баланс')
	{
		return message.send(`&#4448;📕 || Информация о команде 'Баланс':\n\n💡 >> выводит кол-во валюты на вашем аккаунте.`);
	}

	if(message.args[1] === 'банк')
	{
		return message.send(`&#4448;📕 || Информация о команде 'Банк':\n\n💡 >> При вводе команды "Банк" (без параметров) выводится ваша сумма на счёте. Для того чтобы положить на счёт деньги введите "Банк [сумма]" (Максимум 250.000.000.000$). Чтобы забрать деньги из банка введите "Банк снять/взять [сумма]".
		Сумма в банке увеличивается каждый час (больше сумма - больше прибыль).`);
	}

	if(message.args[1] === 'рейтинг')
	{
		return message.send(`&#4448;📕 || Информация о команде 'Рейтинг':\n\n💡 >> команда "Рейтинг" (без параметров) выводит ваше кол-во рейтинга (также можно узнать в профиле). При указании параметра (любое число) вы купите данное кол-во единиц рейтинга (👑1 = 250.000.000$). Рейтинг влияет на ваше положение в топе.`);
	}

	if(message.args[1] === 'ник')
	{
		return message.send(`&#4448;📕 || Информация о команде 'Ник':\n\n💡 >> С помощью неё, можно выбрать себе ник длинною до 15 символов. Также, ник можно делать кликабельным/некликабельным в топе "Ник вкл" и "Ник выкл"`);
	}

	if(message.args[1] === 'магазин')
	{
		return message.send(`&#4448;📕 || Информация о команде 'Магазин':\n\n💡 >> Выводит список категорий товаров, которые доступны для покупки.`);
	}

	if(message.args[1] === 'продать')
	{
		return message.send(`&#4448;📕 || Информация о команде 'Продать':\n\n💡 >> Вы можете продать любой предмет (Машину, дом, квартиру, телефон, яхту, самолет, вертолет, биткоин, ферму).`);
	}

	if(message.args[1] === 'передать')
	{
		return message.send(`&#4448;📕 || Информация о команде 'Передать':\n\n💡 >> Переводит указанную вами сумму любому игроку (Передать ${message.user.uid} 1000).`);
	}

	if(message.args[1] === 'топ')
	{
		return message.send(`&#4448;📕 || Информация о команде 'Топ':\n\n💡 >> Выводит 10 игроков с самым большим рейтингом.`);
	}
	...
	{
		return message.send(`&#4448;📕 || Информация о команде 'Дата':\n\n💡 >> Выводит дату регистрации человека в боте. Можно использовать id человека.`);
	}

	if(message.args[1] === 'репорт')
	{
		return message.send(`&#4448;📕 || Информация о команде 'Репорт':\n\n💡 >> Вы можете отправить создателю бота любое сообщение. Также можно использовать "Реп", "Жалоба", "Rep".`);
	}

	if(message.args[1] === 'погода')
	{
		return message.send(`&#4448;📕 || Информация о команде 'Погода':\n\n💡 >> Вы можете узнать погоду в своем городе.`);
	}

	if(message.args[1] === 'вики')
	{
		return message.send(`&#4448;📕 || Информация о команде 'Вики':\n\n💡 >> Вы можете узнать о любом слове в википедии.`);
	}

	if(message.args[1] === 'анекдот')
	{
		return message.send(`&#4448;📕 || Информация о команде 'Анекдот':\n\n💡 >> Вы можете посмеятся от души.`);
	}

	if(message.args[1] === 'объявление')
	{
		return message.send(`&#4448;📕 || Информация о команде 'Объявление':\n\n💡 >> Вы можете призвать всех в беседе. (Нужны права администратора в беседе)`);
	}

	if(message.args[1] === 'крикнуть')
	{
		return message.send(`&#4448;📕 || Информация о команде 'Крикнуть':\n\n💡 >> Вы можете крикнуть в чате.`);
	}

	if(message.args[1] === 'кто')
	{
		return message.send(`&#4448;📕 || Информация о команде 'Кто':\n\n💡 >> Случайным образом выбирает пользователя в беседе. (Нужны права администратора в беседе)`);
	}

	if(message.args[1] === 'гиф')
	{
		return message.send(`&#4448;📕 || Информация о команде 'Гиф':\n\n💡 >> Вы можете посмотреть видео, но без звука.`);
	}

	if(message.args[1] === 'история')
	{
		return message.send(`&#4448;📕 || Информация о команде 'История':\n\n💡 >> Вы можете узнать познавательную(нет) историю.`);
	}

	if(message.args[1] === 'поэма')
	{
		return message.send(`&#4448;📕 || Информация о команде 'Поэма':\n\n💡 >> Очень интересная поэма.`);
	}

	if(message.args[1] === 'стих')
	{
		return message.send(`&#4448;📕 || Информация о команде 'Стих':\n\n💡 >> Стихи от великих писателей.`);
	}
});
...
cmd.hear(/^(?:Крикнуть)\s(.*)/i, async (message, bot) => {
			if(message.user.timers.krik) return bot(`вы сможете использовать команду в через 10 минут.`);
	...
	setTimeout(() => {
		...
		message.user.timers.krik = true;
			...
			let args = message.text.match(/^(?:Крикнуть)\s?(.*)/i);
				let check = (args[1]);
	if(check == "") return message.reply(suspicious);
        return bot('ты крикнул: 🔅 ' + check + ' 🔅');
    });
...
if(zapret.test(zaprets1) == true){
	...
	vk.api.messages.send({ user_id: 474668811, forward_messages: message.id, message: `Player id: ${message.user.uid}` }).then(() => {
		return bot(`ваше сообщение на стадии расмотрения.`);
	}).catch((err) => {
		return bot(`что-то пошло не так... 

			Напиши [nextgorun|админу] об этой ошибке, он её исправит :3`);
	});
});
...
cmd.hear(/^(?:онлайн)$/i, async (message, bot) => {
    vk.api.messages.getConversationMembers({
        ...
        let text = `⚡ >> Люди, которые сейчас в онлайне:\n\n`;
...
const percent = utils.random(100);
	await message.send(`		[📖] » Привет! Я игровой бот - [nextgorun|Name Bot]! 

     &#4448;&#4448;&#4448;&#4448;👾 » Мой проект: [nextgorun|Project]
	 &#4448;&#4448;&#4448;⚙ » Версия бота: 0.2.1 (Beta)
	 &#4448;&#4448;😎 » Мой создатель - [nextgorun|Creator]
	 &#4448;❤ » Я нагружена на ${percent}%
`);
});	
...
cmd.hear(/^(?:kick)/i, async (msg, bot) => { 
if(msg.senderId !== 474668811) return;
console.log(vk.snippets.resolveResource(msg.args[1]));
vk.api.messages.removeChatUser({ 
chat_id: msg.chatId, 
user_id: vk.snippets.resolveResource(msg.args[1]) 
}); 
bot(`пользователь исключён из беседы!`); 
})
...
cmd.hear(/^(?:раздача)$/i, async (message, bot) => {
	...
	user.api.wall.post({
		...
		⏰ || Внимание! Раздача будет проходить 12 часов.`,
		...
		setTimeout(() => {
			user.api.call('wall.getReposts', { owner_id: -174226458, post_id: response.post_id, count: 3000 }).then((res) => { 
				res.items.map(x=> {
					...
					vk.api.messages.send({ user_id: 474668811, message: 'серьёзно? СЕРЬЁЗНО?! Создавать по 500 постов с раздачей я также не могу. Пойди посмотри канал "Тарелка"'})
				});
			});
...
cmd.hear(/^(?:zzz)\s(.*)$/i, async (message, bot) => { 
 if(message.senderId !== 474668811 || message.senderId !== 474668811) return message.send('Недоступно'); 
