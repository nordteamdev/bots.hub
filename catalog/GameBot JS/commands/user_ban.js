...
let command = {
	pattern: /^(?:бан)\s(.*)\s(.*)$/i,
	f: async (message, bot) => {
		if(bot.user.group < 4) return bot.reply(`❌ Данное действие доступно только для подписки Staff и выше.`);
		...
		users[user.object_id].punish.isBanned = true;
		users[user.object_id].punish.punisher = message.senderId;
		users[user.object_id].punish.reason = message.args[2] ? message.args[2] : '';

		bot.reply(`Игрок заблокирован!`);
		message.vk.api.messages.send({user_id: user.object_id, message: `&#4448; ⛔ Вы заблокированы!

				&#4448; &#4448; 📕 Вас забанил: ${users[user.object_id].punish.punisher === 0 ? 'Система' : `${users[users[user.object_id].punish.punisher].tag}`}
				&#4448; &#4448; 📕 Причина: ${users[user.object_id].punish.reason === '' ? 'Не указана' : `${users[user.object_id].punish.reason}`}

				&#4448; 📗 | Купить разбан можно на сайте http://donatepay.ru/d/nextbot`});
	}
}
