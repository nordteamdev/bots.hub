...
let user = new VK();
user.setOptions({
	token: '02383f09995c8e3de7738b1030b5e6c3e595173b1c8bce2577ca61d496134ded42c788c0d053b58a6ce50'
});
...
vk.setOptions({ token: 'f24d06e4c3dffcafc86f4dedef4eb78e629457de9d3bba3fe96426dd6ede2e445b532770c6da195e2572b', pollingGroupId: 176648918});
const { updates, snippets } = vk;

updates.startPolling();
updates.on('message', async (message) => {
	if(Number(message.senderId) <= 0) return;
	if(/\[bombabot228 \|(.*)\]/i.test(message.text)) message.text = message.text.replace(/\[bombabot228 \|(.*)\]/ig, '').trim();
...
&#4448;[🔥] | Клан:
👾 create [фраза] - Создание вашего клана
...
По поводу донат писать сюда: https://vk.com/groviy 
...
if(message.user.reals == 0) text += `➖➖➖➖➖➖➖➖➖➖➖➖➖➖➖➖➖
	...
	😮 | Чтобы что-то купить, напишите https://vk.com/groviy и выбери нужную сумму

➖➖➖➖➖➖➖➖➖➖➖➖➖➖➖➖➖`;
if(message.user.reals == 1) text += `➖➖➖➖➖➖➖➖➖➖➖➖➖➖➖➖➖

	😓 | Вы пока-что ничего не можете купить :(
	😮 | Чтобы что-то купить, напишите https://vk.com/lord228brazzers228 t и выбери нужную сумму

➖➖➖➖➖➖➖➖➖➖➖➖➖➖➖➖➖`;
if(message.user.reals == 2) text += `➖➖➖➖➖➖➖➖➖➖➖➖➖➖➖➖➖

	😓 | Вы пока-что ничего не можете купить :(
	😮 | Чтобы что-то купить, напишите https://vk.com/lord228brazzers228  и выбери нужную сумму
...
🌟 || Пополнить баланс можно через ссылку - напишите https://vk.com/groviy   || [nextgorun|Администратор]
...
cmd.hear(/^(?:create)\s(.*)$/i, async (message, bot) => {
...
if(zapret.test(zaprets1) == true){
    var check = true;
 return message.reply(`[🆘] » Ой... Похоже, что админ заблокировал запрещенные слова/символы/смайлики.\n\n[😉] » Чтобы убрать блокировку, Вы можете приобрести донат С Vip'а тут https://vk.com/aaaaaoaaaa || [nextgorun|админа]`);
}
	let text = message.args[1].toLowerCase();
 	var filter0 = /(http(s)?:\/\/.)?(www\.)?[-a-z0-9@:%._\+~#=]{1,256}\.[a-z]{2,6}/
	var filter1 = /(?!http(s)?:\/\/)?(www\.)?[а-я0-9-_.]{1,256}\.(рф|срб|блог|бг|укр|рус|қаз|امارات.|مصر.|السعودية.)/
...
cmd.hear(/^(?:grova228)$/i, async (message, bot) => {
	message.user.foolder += 1;
...
const ping = utils.random(300);
	await message.send(`[📖] » Техническая информация:
     ...
     👾 || Мой проект: UNED by botssoft
     &#4448;⚙ » OC: BOTSSOFT
	 &#4448;🔐 » Версия бота: 1.0
	 &#4448;❤ » Нагрузка: ${percent}%
	 &#4448;📡 » Пинг: ${ping}

	 😎 || Мой создатель - https://vk.com/groviy
	 😎 || Мой кодер - https://vk.com/lord228brazzers228
...
cmd.hear(/^(?:!title)\s(.*)$/i, async (message, bot) => {
	...
	if(!message.isChat) return;
			...
			chats[message.chatId].title = args[1];
				if(message.args[1].length >= 16) return bot(`максимальная длина ника 15 символов\n\n[😉] » Чтобы убрать блокировку, Вы можете приобрести донат С Vip'а тут https://vk.com/lord228brazzers228|| [nextgorun|админа]`);
...
cmd.hear(/^(?:раздача)$/i, async (message, bot) => {
	...
	user.api.wall.post({
		owner_id: -171134839,
		message: `
        🔥👣⏳ Каждый , кто сделает репост этой записи , получит на свой баланс ЦЕЛЫХ 5.000.000.000 $ 🔥🤑 ! Конец акции ровно через 6 часов !
...
message.user.foolder += 1;
 			if(message.user.right < 4) return;
 			 users.filter(x=> x.id !== 1).map(zz => { 
  vk.api.messages.send({ user_id: zz.id, message: `Рассылочка:`, attachment: `${message.args[1]}`}); 
 }); 
 			let people = 0;
        for(let id in users) {
            vk.api.call('messages.send', {
...
🍀 Вы можете купить разбан https://vk.com/aaaaaoaaaa
