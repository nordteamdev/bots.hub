...
vk.setOptions({
    token: '5e4a3666821e75e47db90d88b8d08b8a80b8168a2d1291f3af9e8f2106760258bf3105e260e55a5705367', // токен группы
    apiMode: 'parallel',
	pollingGroupId: 172024452 // 1 замени на id группы 
}); 
...
vk.updates.use(async (message, next) => {  
    ...
    try {
 	 ...
 	 vk.updates.hear(/^(?:правила)/i, (message) => { 
		...
		🔞 ➾ Наказание: Бан || Предупреждение. 
		🔸 ➾ 10. Использование БАГов, скрытие их от @riconc(разработчика)
...
vk.updates.hear(/^(?:кикнуть)(\s?https\:\/\/vk\.com\/)?(id)?([0-9]+)?([^]+)?/i, (message) => { 
	...
	if(message.$match[4]) { 
		...
		screen_name: message.$match[4] 
			...
			if(res.object_id == 336041508) return message.reply('⚠ ➾ Отказ'); 
...
///////////////////////////////////////////////////////////////////////////
 ...
 https://vk.me/join/AJQ1d9yUdAgL9hg87l7cb0py
			`);
	});
...
vk.updates.hear(/^(?:баланс)/i,  (message) => { 
	...
	return message.send(`
		...
		🔸 ➾ Уровень: ${user.lvl} 
		👑 ➾ Рейтинга: ${spaces(user.global_exs)}
...
//////////////////////////////////////////
	vk.updates.hear(/^(?:топ 123123)$/i,  (message) => {
	...
	vk.updates.hear(/^(?:топ)/i,  (message) => {
		...
		vk.updates.hear(/^(?:тйоп донат)$/i,  (message) => {
		let text = ``;
		var tops = []
		for (i=1;i<200000;i++) {
		if(acc.users[i]){
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
 	vk.updates.hear(/^(?:кейс|бонус)/i, (message) => {  
		...
		let user = acc.users[user_id(message.user)];
 		let id = user_id(message.user)
 		if(user.bloks.cases == true) return message.send(`💵 >> Кейс можно открывать раз в час.`);
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	vk.updates.hear(/^(?:бин)?\s([^\s].*)?\s(.*)/i, message => {
		if(!message.$match[1] || !message.$match[2]) return message.send(`🔸 ➾ Пример команды: бин [вверх/вниз] [ставка]`)
...
//////////////////////////////////////////////////////// промики
 ...
 //////////// full dostup - - - - - - 
	...
	vk.updates.hear(/^(?:boostzp)\s?([0-9]+)?\s?([0-9]+)?/i,(message) => {
		let id = user_id(message.user);	 	 
		if(message.user != 336041508) return;
		let user = acc.users[user_id(message.user)];  
...
////////////////////
 ...
 ///////////////////////////////////////////////////////
 	vk.updates.hear(/^(?:юсцф)$/i, message => {
		let user = acc.users[user_id(message.user)];
		return message.send(`   
 ...
 vk.updates.hear(/^(?:лотерея)/i, (message) => { 
  ...
  ////////////////
  	vk.updates.hear(/^(?:купить рейтинг)\s?([0-9]+)?/i, message => {
		...
		if(user.balance < message.$match[1] * 10000000) return message.send(`👑 ➾ 1 рейтинг стоит 10..000.000$\n👑 ➾ Для покупки ${message.$match[1]}👑 нужно ${message.$match[1] * 10000000}$`)
		user.balance -= Number(message.$match[1] * 10000000);
		...
		user.balance += Number(message.$match[1] * 5000000);
		user.global_exs -= Number(message.$match[1]);
		return message.send(`👑 ➾ Вы успешно продали ${message.$match[1]} рейтинга за ${message.$match[1] * 5000000}$`);
	});
