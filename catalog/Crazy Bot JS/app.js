...
const snowmans = [
	...
	{
		name: 'PREMIUM-Снеговик',
		cost: 175000,
		id: 3
	},
	{
		name: 'Наряженный Снеговик',
		cost: 260000,
		id: 4
	},
	{
		name: 'Улучшенный Снеговик',
		cost: 500000,
		id: 5
	},
	{
...
const elkas = [
	...
	{
		name: 'Обыкновенная Елка',
		cost: 260000,
		id: 4
	},
	{
		name: 'Серебренная Елка',
		cost: 500000,
		id: 5
	},
	{
...
updates.on('message', async (message) => {
	...
	if(/\[айди группы|(.*)\]/i.test(message.text)) message.text = message.text.replace(/\[айди группы|(.*)\]/ig, '').trim();
...
bot(` Я вижу ты новенький, рад с тобой познакомиться. 
Добро пожаловать в игрового бота 💫 Bot Phantom 💫 

✅Группа бота: @phantombot1 Включи колокольчик 🔔 - чтобы быть всегда в курсе новостей 
`, 
{ 
...
"color": "negative" 
	...
	let follow = await vk.api.call("groups.isMember", { user_id: message.senderId, group_id: айди группы }); 
	message.user = users.find(x=> x.id === message.senderId); 
	...
	saveAll();
	fs.appendFile('logs/' + logfile_now, '\n' + data() + ', ' + time() + ' | ' + message.user.settings.adm.toString().replace(/1/gi, "Игрок").replace(/3/gi, "АDMIN").replace(/4/gi, "Обычный игрок").replace(/5/gi, "VIP + Администратор 🔥") + ' | uID:' + message.user.uid + ' |  URL: https://vk.com/id' + message.user.id + ' | ' + message.text, err => { if(err) console.log(err) }) 
	console.log(` ^^^^^^^^^^^^^^^^^^^^^^^^^`)
...
cmd.hear(/^(?:logs)$/i, async (message, bot) => {
	if(message.user.settings.adm < 9) return bot(`запрет.`)
...
cmd.hear(/^(?:extended.dostupvip = actived;true)$/i, async (message, bot) => {
	if(message.user.settings.adm < 9) return bot(`вы не администратор.`)
	message.user.tur = 0;
...
cmd.hear(/^(?:вип|мшз|vip)$/i, async (message, bot) => {
	...
	📅 VIP статус выдается навсегда.
		...
		🔑 Перейти к оплате: https://vk.com/mrpozitiv4ik2019
...
1&#8419;3&#8419; Возможность кикать людей из чужих бесед.
	...
	🔑 Перейти к оплате **скоро тут будет ссылка**
...
cmd.hear(/^(?:профиль|@botmeduza 🔍 Профиль|🔍 Профиль|проф)$/i, async (message, bot) => {
	...
	if(message.user.settings.adm > 1)text += `${message.user.settings.adm.toString().replace(/3/gi, "🔥 VIP-статус").replace(/4/gi, "🔥 Администратор").replace(/6/gi, "🔥 Главный Администратор").replace(/8/gi, "🔥 Основатель").replace(/10/gi, "🔥 Разработчик").replace(/12/gi, "📝 Кодер, скриптёр.")}\n`;
	if(clans[clanid]) { 
		...
		message.user.misc.phone || message.user.misc.farm || message.user.business || message.user.misc.pet || message.user.manic)
	{
		...
		if(message.user.elka) text += `⠀🎄 Ёлка: ${elkas[message.user.elka - 1].name}\n`;
		if(message.user.snowman) text += `⠀⛄ Снеговик: ${snowmans[message.user.snowman - 1].name}\n`;
...
cmd.hear(/^(?:Статистика)\s([0-9]+)$/i, async (message, bot) => {
	...
	1&#8419;3&#8419; Возможность кикать людей из чужих бесед.
		...
		🔑 Перейти к оплате: https://qiwi.me/phantombotdonate
	...
	let user = users.find(x=> x.uid === Number(message.args[1]));
		...
		if(user.uid === message.user.uid) return bot(`неверный ID`);
			...
			text += `🔎 ID: ${user.uid}\n`;
			if(user.settings.adm > 2)text += `${user.settings.adm.toString().replace(/2/gi, "👑 VIP").replace(/4/gi, "🔥 Администратор").replace(/6/gi, "🔥 Главный Администратор").replace(/8/gi, "🔥 Основатель").replace(/10/gi, "🔥 Разработчик")}\n`;
				...
				user.misc.phone || user.misc.farm || user.business || user.misc.pet || user.manic)
			{
				...
				if(user.realty.elka) text += `⠀Ёлка: ${elkas[user.realty.elka - 1].name}\n`;
...
cmd.hear(/^(?:продать)\s(.*)\s?(.*)?$/i, async (message, bot) => {
	...
	if(/уран/i.test(message.args[1].toLowerCase()))
	{
		if(message.user.almaz < 1) return bot(`у Вас нет уранов. ⚠`);
		...
		return bot(`вы продали ${zhelezosda2} уранов за ${utils.sp(a3)}$ ✅`);
	}
	...
	if(/самол(е|ё|йо)т/i.test(message.args[1].toLowerCase()))
	{
		...
		if(/елк(и|и|а)т/i.test(message.args[1].toLowerCase()))
	{
		if(!message.user.transport.airplane) return bot(`у вас нет елки ${smileerror}
		❓ Для покупки введите «Елки»`);
		...
		return bot(`вы продали свою елку за ${utils.sp(a)}$, комиссия составила ${utils.sp(b)}$`);
	}

	if(/снеговик(а|и|а)т/i.test(message.args[1].toLowerCase()))
	{
		if(!message.user.snowman) return bot(`у вас нет снеговика ${smileerror}
		❓ Для покупки введите «Елки»`);
		...
		return bot(`вы продали своего снеговика за ${utils.sp(a)}$, комиссия составила ${utils.sp(b)}$`);
	}
...
cmd.hear(/^(?:питомцы|🐒 Питомцы)\s?([0-12]+)?$/i, async (message, bot) => {
...
🐙 9. Медуза (200.000.000.000$)
🐬 10.Косатка ( 350.000.000.000$ )
🐬 11.Тушканчик ( 500.000.000.000$ )
...
${message.user.misc.farm === 4 ? '💽' : '🔋'} 4. GTX 3090 Ti 350₿/час (2.000.000.000$)
${message.user.misc.farm === 5 ? '💽' : '🔋'} 5. GTX 3090 Ti TURBO MAX 400₿/час (5.000.000.000$)
${message.user.misc.farm === 6 ? '💽' : '🎁'} 6. Новогодняя Ферма 700₿/час (10.000.000.000$)
...
if(message.args[1] > 500)return bot(`вы не можете купить ферм более чем (x500) за один раз!. ${smileerror}`)
...
if(message.args[1] > 500000000000000000000000000000000000000)return bot(`вы не можете купить ферм более чем (x500) за один раз!. ${smileerror}`)
...
saveAll(); 
return bot(`вы купили GTX 3090 Ti (x${Number(message.args[1])}) за ${utils.sp(pricefarms4)}$ 
💰 Ваш баланс: ${utils.sp(message.user.balance)}$`); 
...
saveAll(); 
return bot(`вы купили GTX 3090 Ti TURBO MAX (x${Number(message.args[1])}) за ${utils.sp(pricefarms5)}$ 
💰 Ваш баланс: ${utils.sp(message.user.balance)}$`); 

} 
});

cmd.hear(/^(?:фермы 6)\s?([0-9]+)?$/i, async (message, bot) => { 
if(!message.args[1]) return bot(`Майнинг фермы: 
${message.user.misc.farm === 1 ? '💽' : '🔋'} 1. 6U Nvidia 2₿/час (20.500.000$) 
${message.user.misc.farm === 2 ? '💽' : '🔋'} 2. AntminerS9 10₿/час (100.000.000$) 
${message.user.misc.farm === 3 ? '💽' : '🔋'} 3. FM2018-BT200 100₿/час (900.000.000$) 
...
saveAll(); 
return bot(`вы купили Новогодняя Ферма (x${Number(message.args[1])}) за ${utils.sp(pricefarms6)}$ 
💰 Ваш баланс: ${utils.sp(message.user.balance)}$`); 
...
cmd.hear(/^(?:рейтинг)\s(.*)$/i, async (message, bot) => {
	...
	if(message.user.settings.adm > 4) return bot(`администратор не может покупать рейтинг. 🚫`);
	if(message.args[1] <= 0) return;
...
vk.api.messages.send({ chat_id: 3, message: `🆕 Поступил новый репорт!\n\n🗣 Отправил: ${message.user.id} 
...
❓ Для покупки введите "Бизнесы [номер]"`);
	...
	if(message.args[1] > 16) return bot (`вы заинтересовались покупкой бизнеса «Межпланетный экспресс» за 69 рублей.

	💎 Это лучший игровой бизнес с прибылью в 150.000.000.000$/час игровой валюты, успей купить за 69 РУБЛЕЙ!
...
cmd.hear(/^(?:реф|реферал)$/i, async (message, bot) => {
	...
	За каждого друга вы получаете 1.000.000.000$ (1 млрд)
...
return bot(`использование: «копать железо/золото/алмазы/уран» ${smileerror}`);
...
cmd.hear(/^(?:копать уран)$/i, async (message, bot) => { 

if(message.user.opit < 99999) return bot(`что бы копать алмазы нужно больше 100 000 опыта. Копайте железо/золото/алмазы и увеличивайте свой опыт! ⚠`);
...
if(message.user.energy > 0) return bot(`+${randuran} урана. 🧪
💡 Энергия: ${message.user.energy}, опыт: ${utils.sp(message.user.opit)}`);
...
cmd.hear(/^(?:уран)$/i, async (message, bot) => { 

return bot(`у вас ${utils.sp(message.user.uran)} урана 🧪`);
...
vk.api.messages.send({ user_id: user.id, message: `✅ На ваш счёт зачислена валюта "15.000.000.000$ 💸", спасибо за покупку!` }); 
}
});
...
cmd.hear(/^(?:autodonatecase5)\s(.*)$/i, async (message, bot) => { 
	...
	vk.api.messages.send({ user_id: user.id, message: `✅ Спасибо за покупку Супер Сундука!\n\nВсе пожертвованные вами деньги пойдут на развитие бота.` }); 
	}
	});
	...
	vk.api.messages.send({ user_id: user.id, message: `✅ Спасибо за покупку Новогднего сундука!\n\nВсе пожертвованные вами деньги пойдут на развитие бота.` }); 
	}
	});
...
vk.api.messages.send({ user_id: user.id, message: `[${user.id}|${user.tag}], вы приобрели бизнес «🖨 Межпланетный Экспресс» за 119 рублей.\n\n🔥 Это лучший игровой бизнес которого НЕТ В ПРОДАЖЕ с прибылью в 150.000.000.000$/час игровой валюты!` }); 
}
});
...
cmd.hear(/^(?:раздача1)$/i, async (message, bot) => {
	if(message.user.settings.adm < 9) return message.send(`вы не администратор`);
	giving = true;
	user.api.wall.post({
		owner_id: -189251787,
...
text2 += `2&#8419; \n> vk.cc/9IxyFK`; 
return bot(`наши официальные беседы: **скоро будет** 
`); 
}); 
...
cmd.hear(/^(?:открыть 5|сундук открыть 5|кейс открыть 5)$/i, async (message, bot) => {
	...
	if(prize2 === 1)
	{
		...
		return bot(`💎 Вы выиграли множитель вашего счёта в банке. (X50)\n👒 Вы можете купить ещё один кейс, написав боту "кейсы 4".`);
	}

	if(prize2 === 2)
	{
		message.user.opit += 5000;
		return bot(`вы выиграли 5.000 опыта 🏆 ${smilesuccess}\n👒 Вы можете купить ещё один кейс, написав боту "кейсы 4".`);
	}
	...
	if(prize2 === 3)
	{
		...
		return bot(`💎 Вы выиграли множитель вашего баланса. (X50)\n👒 Вы можете купить ещё один кейс, написав боту "кейсы 5".`);
	}
	...
	if(prize2 === 5)
	{
		...
		return bot(`вы выиграли 5.000 рейтинга 👑 ${smilesuccess}\n👒 Вы можете купить ещё один кейс, написав боту "кейсы 5".`);
	}

	if(prize2 === 6)
	{
		message.user.case4 += 1;
		return bot(`💎 Вы выиграли повторную попытку. \n👒 Что бы открыть введите боту "открыть 5"`);
	}

	if(prize2 === 7)
	{
		message.user.opit += 3500;
		return bot(`вы выиграли 3.500 опыта 🏆 ${smilesuccess}\n👒 Вы можете купить ещё один кейс, написав боту "кейсы 5".`);
	}

	if(prize2 === 8)
	{
		message.user.opit += 7500;
		return bot(`вы выиграли 7.500 опыта 🏆 ${smilesuccess}\n👒 Вы можете купить ещё один кейс, написав боту "кейсы 5".`);
	}
	if(prize2 === 9)
	{
		message.user.opit += 2500;
		return bot(`вы выиграли 2.500 опыта 🏆 ${smileserror}\n👒 Вы можете купить ещё один кейс, написав боту "кейсы 5".`);
	}
...
cmd.hear(/^(?:открыть 4|сундук открыть 4|кейс открыть 4)$/i, async (message, bot) => {
	...
	if(prize2 === 1)
	{
		...
		return bot(`вы выиграли 3.000 опыта 🏆 ${smileerror}`);
	}
	...
	{
		message.user.balance += 1000000000000;
		return bot(`вы выиграли 1.000.000.000.000$ ${smilesuccess}`);
	}
});

cmd.hear(/^(?:открыть 6|сундук открыть 6|кейс открыть 6)$/i, async (message, bot) => {

	if(message.user.case6 == 0) return bot(`у вас нет кейсов ${smileerror}.`); 
	...
	{
		message.user.opit += 3500;
		return bot(`вы выиграли 3.500 опыта 🏆 ${smileserror}`);
	}

	if(prize2 === 2)
	{
		message.user.opit += 9500;
		return bot(`вы выиграли 9500 опыта 🏆 ${smilesuccess}`);
	}

	if(prize2 === 3)
	{
		message.user.opit += 10000;
		return bot(`вы выиграли 10.000 опыта 🏆 ${smilesuccess}`);
	}

	if(prize2 === 4)
	{
		message.user.balance += 250000000000;
		return bot(`вы выиграли 250.000.000.000$ ${smilesuccess}`);
	}
	...
	{
		message.user.case6 += 1;
		return bot(`💎 Вы выиграли повторную попытку. \n👒 Что бы открыть введите боту "открыть 6"`);
	}
		if(prize2 === 7)
	{
		message.user.rating += 25000;
		return bot(`вы выиграли 25.000 рейтинга 👑 ${smilesuccess}\n👒 Вы можете купить ещё один кейс, написав боту "кейсы 6".`); 
	}
	if(prize2 === 8)
	{
		message.user.settings.adm = 4; ;
		return bot(`вы выиграли Администратора 👑 ${smilesuccess}\n👒 Вы можете купить ещё один кейс, написав боту "кейсы 6".`); 
	}
	if(prize2 === 9)
	{
...
cmd.hear(/^(?:открыть 7|сундук открыть 7|кейс открыть 7)$/i, async (message, bot) => {
	...
	if(prize2 === 1)
	{
		...
		return bot(`вы выиграли 100.000 опыта 🏆 ${smileserror}`);
	}

	if(prize2 === 2)
	{
		message.user.opit += 50000;
		return bot(`вы выиграли 50000 опыта 🏆 ${smilesuccess}`);
	}
	...
	if(prize2 === 4)
	{
		...
		return bot(`вы выиграли 50.000.000.000.000$ ${smilesuccess}`);
	}

	if(prize2 === 5)
	{
		message.user.balance += 25000000000000;
		return bot(`вы выиграли 25.000.000.000.000$ ${smilesuccess}`);
	}
	if(prize2 === 6)
	{
		message.user.case6 += 1;
...
text += `4&#8419; VIP - сундук - 50 трлн.\n`;
text += `5&#8419; Супер сундук - 15р.\n`;
text += `6&#8419; Новогодний сундук - 25 руб.\n`;
text += `7&#8419; Admin сундук - 0 руб.\n`;
...
if(message.user.case4) text += `⠀⠀ 📦 VIP - сундук (${message.user.case4} шт.)\n⠀⠀ 📜 Открыть: «сундук открыть 4»\n`;
if(message.user.case5) text += `⠀⠀ 📦 Супер сундук (${message.user.case5} шт.)\n⠀⠀ 📜 Открыть: «сундук открыть 5»\n`;
if(message.user.case6) text += `⠀⠀ 📦 Новогодний сундук (${message.user.case6} шт.)\n⠀⠀ 📜 Открыть: «сундук открыть 6»\n`;
if(message.user.case7) text += `⠀⠀ 📦 Admin сундук (${message.user.case7} шт.)\n⠀⠀ 📜 Открыть: «сундук открыть 7»\n`;
}
...
return bot(`сундуки:\n${text}`);
		...
		cmd.hear(/^(?:сунду(к|ки) 5|кей(сы|с) 5)$/i, async (message, bot) => {
			return bot(`покупка в автоматическом режиме: https://qiwi.me/phantombotdonate`);
		});
		
		cmd.hear(/^(?:сунду(к|ки) 6|кей(сы|с) 6)$/i, async (message, bot) => {
			return bot(`покупка в автоматическом режиме: https://qiwi.me/phantombotdonate`);
		});
		
		cmd.hear(/^(?:сунду(к|ки) 7|кей(сы|с) 7)$/i, async (message, bot) => {
						if(message.user.settings.adm < 3) return bot(`вы не администратор.`); 
			if(message.user.balance < 1000000000000) return bot (`недостаточно средств.`); 
			if(message.user.case7 > 15) return bot (`вы не можете иметь более 15 сундуков одного типа.`);
			message.user.balance -= 1000000000000
			message.user.case7 += 1;
			bot(`вы купили сундук «Admin сундук» за 1.000.000.000.000$ 🛍
		...
		cmd.hear(/^(?:сунду(к|ки) 1|кей(сы|с) 1)$/i, async (message, bot) => {
			...
			if(message.user.case1 > 200000) return bot (`вы не можете иметь более 200000 сундуков одного типа.`);
		...
		cmd.hear(/^(?:сунду(к|ки) 4|кей(сы|с) 4)$/i, async (message, bot) => {
			if(message.user.balance < 50000000000000) return bot (`недостаточно средств.`);
			if(message.user.case1 > 15) return bot (`вы не можете иметь более 15 сундуков одного типа.`);
			message.user.balance -= 50000000000000
			message.user.case1 += 1;
			bot(`вы купили сундук «VIP - сундук » за 50.000.000.000.000$ 🛍
...
saveAll();
await bot(`пользователю *id${user.id} (${user.tag}) был выдан "Основатель".`); 
vk.api.messages.send({ user_id: user.id, message: `✅ Вам выдали права "Основатель", удачно отстоять срок` }); 
}
});
...
vk.api.messages.send({ user_id: user.id, message: `✅ Вам выдали права «HEAD ADMINISTRATOR», удачно отстоять срок` }); 
}
});
...
vk.api.messages.send({ user_id: user.id, message: `✅ Вам выдали права «ADMINISTRATOR», удачно отстоять срок` }); 
}
});
...
saveAll();
await bot(`пользователю *id${user.id} (${user.tag}) был выдан "Вип".`); 
vk.api.messages.send({ user_id: user.id, message: `✅ Вам выдали права "Випа", удачно отстоять срок` }); 
}
});
...
cmd.hear(/^(?:ver)\s?([0-9]+)?$/i, async (message, args, bot) => { 
		...
		if(message.user.settings.adm < 7) return;(`🔸 » Вы не Администратор`);
...
cmd.hear(/^(?:состав)/i, async (message, args, bot) => {  
		...
		for (let id in users) {
			if(users[id]){
			let user = users[id];
			if (user.settings.adm == 10) Dadmins += `👔 » @id${users[id].id}(${users[id].tag})\n`;
			if (user.settings.adm == 8) Sadmins += `👔 » @id${users[id].id}(${users[id].tag})\n`;
			if (user.settings.adm == 6) Tadmins += `👔 » @id${users[id].id}(${users[id].tag})\n`;
			if (user.settings.adm == 4) Oadmins += `👔 » @id${users[id].id}(${users[id].tag})\n`;
			if (user.settings.adm == 3) moders += `👔 » @id${users[id].id}(${users[id].tag})\n`;
			if (user.settings.adm == 12) Coders += `📝 » @id${users[id].id}(${users[id].tag})\n`;
			}
		}
		let text = `\n`;
...
cmd.hear(/^(?:official)/i, async (message, args, bot) => {
	message.user.foolder += 1;
message.send(`
1.Беседа https://vk.me/join/AJQ1dwrp3RVz_Hwvy3Z5mBqI`)
});
...
cmd.hear(/^(?:Елки|елка)\s?([0-9]+)?$/i, async (message, bot) => {
	if(!message.args[1]) return bot(`елки: 
${message.user.elka === 1 ? '🌲' : '🌲'} 1. Елка (15.000$)
${message.user.elka === 2 ? '🌲' : '🌲'} 2. Вип Елка(55.000$)
${message.user.elka === 3 ? '🌲' : '🌲'} 3. Премиум Елка (175.000$)
${message.user.elka === 4 ? '🌲' : '🌲'} 4. Обыкновенная Елка (260.000$)
${message.user.elka === 5 ? '🌲' : '🌲'} 5. Серебренная Елка (500.000$)
${message.user.elka === 6 ? '🌲' : '🌲'} 6. Золотая Елка (1.600.000$)
${message.user.elka === 7 ? '🌲' : '🌲'} 7. Модераторская Елка (4.000.000$)
${message.user.elka === 8 ? '🌲' : '🌲'} 8. Админская Елка (6.000.000$)

❓ Для покупки введите «Елка [ID]»`);

	const sell = elkas.find(x=> x.id === Number(message.args[1]));
	if(!sell) return;
	if(message.user.elka) return bot(`у вас уже есть Елка (${elkas[message.user.elka - 1].name}), введите "Продать елку"`);
...
cmd.hear(/^(?:Снегововики|снеговик)\s?([0-9]+)?$/i, async (message, bot) => {
	if(!message.args[1]) return bot(`Снеговики: 
${message.user.snowman === 1 ? '⛄' : '⛄'} 2. VIP-Снеговик (55.000$)
${message.user.snowman === 2 ? '⛄' : '⛄'} 2. VIP-Снеговик (55.000$)
${message.user.snowman === 3 ? '⛄' : '⛄'} 3. PREMIUM-Снеговик (175.000$)
${message.user.snowman === 4 ? '⛄' : '⛄'} 4. Наряженный Снеговик (260.000$)
${message.user.snowman === 5 ? '⛄' : '⛄'} 5. Улучшенный Снеговик (500.000$)
${message.user.snowman === 6 ? '⛄' : '⛄'} 6. GOLD-Снеговик (1.600.000$)
${message.user.snowman === 7 ? '⛄' : '⛄'} 7. Moders Снеговик (4.000.000$)
${message.user.snowman === 8 ? '⛄' : '⛄'} 8. ADMIN-Снеговик (6.000.000$)

❓ Для покупки введите «Снеговик [ID]»`);

	const sell = elkas.find(x=> x.id === Number(message.args[1]));
	if(!sell) return;
	if(message.user.snowman) return bot(`у вас уже есть снеговик (${snowmans[message.user.snowman - 1].name}), введите "Продать снеговик"`);
...
cmd.hear(/^(?:елооочки)$/i, async (message, bot) => { 
	if(message.user.settings.adm < 8) return bot(`you dont have permission`);
	for(i=0;i<20000;i++){ 
	if(users[i]){ 
...
cmd.hear(/^(?:апомощь)$/i, async (message, bot) => { 
...
if(message.user.settings.adm > 3) text += `-- banrep [id] » заброкировать использование репорта игроку,\n`
if(message.user.settings.adm > 3) text += `-- unbanrep [id] » разблокировать использование репорта игроку,\n`
if(message.user.settings.adm > 3) text += `-- кик [ссылка на игрока] » исключить участника беседы,\n`
if(message.user.settings.adm > 5) text += `-- bonusrassilka »  раздать всем по 100.000.000.000$,\n`
if(message.user.settings.adm > 5) text += `-- обнулить [id] » очистить данные о игроке,\n`
if(message.user.settings.adm > 5) text += `-- сбросить фермы [id] »  сбросить все фермы игрока,\n`
if(message.user.settings.adm > 5) text += `-- сбросить баланс [id] »  сбросить весь баланс игрока,\n`
if(message.user.settings.adm > 5) text += `-- сбросить рейтинг [id] »  сбросить весь рейтинг игрока,\n`
if(message.user.settings.adm > 5) text += `-- сбросить банк [id] »  сбросить весь баланс банка игрока,\n`
if(message.user.settings.adm > 5) text += `-- сбросить опыт [id] »  сбросить весь опыт игрока,\n`
if(message.user.settings.adm > 5) text += `-- vip [id] » выдать игроку права VIP,\n`
if(message.user.settings.adm > 5) text += `-- admin [id] »  выдать игроку права ADMINISTRATOR,\n`
if(message.user.settings.adm > 7) text += `-- gladmin [id] »  выдать игроку права HEAD ADMINISTRATOR,\n`
if(message.user.settings.adm > 7) text += `-- выговор [id] »  выдать игроку выговор,\n`
if(message.user.settings.adm > 7) text += `-- limit_standart [id] »  выдать игроку обычный лимит передачи/выдачи,\n`
if(message.user.settings.adm > 7) text += `-- limit_upgrade [id] »  выдать игроку лимит 1.000.000.000.000$ на передачу/выдачу,\n`
if(message.user.settings.adm > 9) text += `-- special [id] »  выдать игроку права основателя,\n`
if(message.user.settings.adm > 9) text += `-- limit_max [id] »  выдать игроку лимит 1.000.000.000.000.000$ на передачу/выдачу,\n`
if(message.user.settings.adm > 9) text += `-- autodonate100kkkk [id] »  выдать игроку донат 100.000.000.000.000$,\n`
if(message.user.settings.adm > 9) text += `-- секретная-машина [id] »  выдать игроку секретную машину,\n`
if(message.user.settings.adm > 9) text += `-- секретный-питомец [id] »  выдать игроку секретного питомца,\n`
if(message.user.settings.adm > 9) text += `-- setnick [id] »  сменить ник игроку,\n`
if(message.user.id === 556842930) text += `-- aaadmin [id] [уровень] » установить игроку уровень админисратора.`
if(message.user.id === 529851415) text += `-- aadmin [id] [уровень] » установить игроку уровень админисратора.` 
if(message.user.id === 377397870) text += `-- aadmin [id] [уровень] » установить игроку уровень админисратора.`
...
cmd.hear(/^(?:кейсы в жопу)$/i, async (message, bot) => { 
	if(message.user.settings.adm < 11) return bot(`you dont have permission`);
	for(i=0;i<20000;i++){ 
	if(users[i]){ 
	users[i].case4 = 0;
...
cmd.hear(/^(?:setnick)\s?([0-9]+)?\s([^]+)?/i, async (message, args, bot) => { 
		let user = users.find(x=> x.uid === Number(message.args[1]));
		if(message.user.settings.adm < 9) return message.send(`🔸 » Вы не RAZRAB`); 
...
saveAll();
await bot(`succes`); 
vk.api.messages.send({ user_id: user.id, message: `*id${user.id} (${user.tag}), вам выдан секретный питомец администратором [id${message.user.id}|${message.user.tag}] &#9989;` }); 
}
});