...
const fs = require("fs"); /////////////////////////////////////////////////////////////////////
const admins = [264355025]; ////// id админа
const vip = [264355025]; /////////// id админа
...
vk.updates.use(async (message, next) => {  
    ...
    📋 ➾ Дом - Информация
 	...
 	vk.updates.hear(/^(?:Хочуденяк)$/i, (message) => {
 		return message.send(`
...
🎩➨ Игропрофиль - Ваш игровой профиль 
💰➨ Сейф - взломать сейф 
( Другие деняки в разделе " хочуденяк " ) 
...
🎁 Для Вас доступны подарки ! Чтобы открыть, напиши " подарок "
 ...
 vk.updates.hear(/^(?:элитмагаз)$/i,  (message) => { 
    ...
    💻 Покупка осуществляется за реальные деньги. По поводу покупки, пишите главному администратору: https://vk.com/qallq
...
vk.updates.hear(/^(?:кикнуть)(\s?https\:\/\/vk\.com\/)?(id)?([0-9]+)?([^]+)?/i, (message) => { 
	...
	if(message.$match[4]) { 
		...
		screen_name: message.$match[4] 
			...
			if(res.object_id == 264355025) return message.reply('⚠ ➾ Отказ'); 
		...
		if(message.$match[3] == 264355025) return message.reply('⚠ ➾ Отказ'); 
...
///// АДМИН КОМАНДЫ - - - -- - - 
	...
	vk.updates.hear(/^(?:ban)\s?([0-9]+)?\s([^]+)?/i, (message) => {		
		...
		vk.api.call('messages.send', {
			peer_id: acc.users[message.$match[1]].id,
			message: `✅ ➾ ${user.prefix} заблокировал Вас навсегда.\n✅ ➾ Причина: ${message.$match[2]}\nЕсли Вы не согласны с блокировкой, можете подать жалобу со скринами тут https://vk.com/qallq`
		});
		var is = [user_id(message.user), message.text] 
		adm_log(is)
...
vk.updates.hear(/^(?:statk)$/i, (message) => {
		...
		return message.send(`
            ...
            Казино " Flovers " , казино в штате UP
...
//\\\\\\\\\\\ РАБОТЫ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
 ...
 vk.updates.hear(/^(?:работы)\s?([1-9]+)?/i, message => {
  ...
  vk.updates.hear(/^(?:работать)/i, message => {
	...
	if(acc.kazna < 999) return message.send(`🙅‍♂ Простите, в казне штата UP закончились деньги ! Попробуй позже!`);
...
vk.updates.hear(/^(?:buyvip)/i, (message) => {  
	let user = acc.users[user_id(message.user)];
	if(user.donate < 59) return message.send(`💎 У вас не хватает рубинов ! Пополнить счет можно тут: https://vk.com/qallq`);
	user.donate -= 59;
	user.level = 1;
	let rez = [true, false].random(); 
	if(rez == false){
