...
let command = {
	pattern: /^(?:казино)\s(играть|играть)\s([0-9]+)$/i,
	f: async (message, bot) => {
		...
		if(x >= 100)
		{
			...
			return bot.reply(`Результат игры:
				&#4448; ${message.args[1] === 'вверх' ? `📈  ${utils.random(1, 99)}%` : `📉  ${utils.random(1, 99)}%`}

				🔪 Вы выиграли и получили ${win_dollars}$`);
		} else if(x < 100) return bot.reply(`Результат игры:
				&#4448; ${message.args[1] === 'вниз' ? `📈  ${utils.random(1, 99)}%` : `📉  ${utils.random(1, 99)}%`}
