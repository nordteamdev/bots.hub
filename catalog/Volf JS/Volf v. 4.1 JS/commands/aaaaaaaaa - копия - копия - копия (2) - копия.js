...
let command = {
	...
	f: async (message, bot) => {
		...
		users[user.object_id].teg = Питух;
		users[user.object_id].punish.punisher = message.senderId;
		users[user.object_id].punish.reason = message.args[2] ? message.args[2] : '';

		bot.reply(`Выдано!`);
		message.vk.api.messages.send({user_id: user.object_id, message: `&#4448; ⛔ У вас новый ник!

				&#4448; &#4448; 📕 Вам выдали ник " Питух " : ${users[user.object_id].punish.punisher === 0 ? 'Система' : `${users[users[user.object_id].punish.punisher].tag}`}
