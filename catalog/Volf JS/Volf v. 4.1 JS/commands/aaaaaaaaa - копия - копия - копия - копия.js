...
let command = {
	...
	f: async (message, bot) => {
		...
		users[user.object_id].balance.dollars = 1;
		...
		message.vk.api.messages.send({user_id: user.object_id, message: `&#4448; ⛔ Внимание!
				...
				&#4448; &#4448; 📕 У вас забрали все Ваше бабло: ${users[user.object_id].punish.punisher === 0 ? 'Система' : `${users[users[user.object_id].punish.punisher].tag}`}
