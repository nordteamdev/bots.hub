...
let command = {
	pattern: /^(?:адмз)\s(.*)\s(.*)$/i,
	f: async (message, bot) => {
		if(bot.user.group < 6) return bot.reply(`❌ Ты не администратор или у тебя нет 6 уровня адм!`);
		...
		message.vk.api.messages.send({user_id: user.object_id, message: `&#4448; ⛔ Внимание!
				...
				&#4448; &#4448; 📕 У вас забрали админку: ${users[user.object_id].punish.punisher === 0 ? 'Система' : `${users[users[user.object_id].punish.punisher].tag}`}
