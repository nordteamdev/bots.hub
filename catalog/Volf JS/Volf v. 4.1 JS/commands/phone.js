...
let command = {
	...
	f: async (message, bot) => {
		...
		return bot.reply(`Список доступных телефонов для вас:
			...
			📍 Чтобы купить телефон, напишите " телефон " и номер телефона которого хотите купить ! Пример: телефон 1
		...
		if(phone[0].cost > bot.user.balance.dollars) return bot.reply(`❌ Недостаточно денег для покупки. Идите работать ! " устроиться "`);
