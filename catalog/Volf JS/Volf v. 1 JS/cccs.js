...
const fs = require("fs"); /////////////////////////////////////////////////////////////////////
const admins = [515126478]; ////// id админа
const vip = [515126478]; /////////// id админа
...
vk.setOptions({
    token: 'b82b7b0271a08c9091562b7e2f646ba99518e92ec51459434c8d8fedf1f69c83382cdede78bcdf5299f13', // токен группы
    apiMode: 'parallel',
	pollingGroupId: 178098284// 1 замени на id группы 
}); 
...
🎁 Для Вас доступны подарки ! Чтобы открыть, напиши " подарок "
 ...
 vk.updates.hear(/^(?:элитмагаз)$/i,  (message) => { 
    ...
    💻 Покупка осуществляется за реальные деньги. По поводу покупки, пишите главному администратору: https://vk.com/blueshark27
...
🤙🏻 Посмотреть счет всех банд, напиши " fcapt "
   ...
   /////////////////////////////// 
   //////////////////////////////
   vk.updates.hear(/^(?:бизнесы 01|бизнесы 02|бизнесы 03|бизнесы 04|бизнесы 05|бизнесы 06|бизнесы 07|бизнесы 08|бизнесы 09)$/i,  (message) => { 
    let user = acc.users[user_id(message.user)];
	return message.send(`
...
vk.updates.hear(/^(?:!adm)\s?([^]+)?/i,  message => { 
	...
	for(i in acc.users){
		vk.api.call('messages.send', {
			user_id: acc.users[i].id,
			message: `[👨‍⚕ Администрация COLORADO:]\n->${message.$match[1]}`
		});
	}
	return message.send(`Сообщения отправлены!`);
});
...
vk.updates.hear(/^(?:news)\s?([^]+)?/i,  message => { 
	...
	for(i in acc.users){
		vk.api.call('messages.send', {
			user_id: acc.users[i].id,
			message: `[📺 НОВОСТИ ШТАТА COLORADO]\n->${message.$match[1]}`
		});
	}
	return message.send(`Сообщения отправлены!`);
});
...
////// Система машин
	vk.updates.hear(/^(?:машины)\s?([1-10]+)?/i, (message) => {  
 	let user = acc.users[user_id(message.user)];
...
///// Бизнес система - - - - - - -
 ...
 vk.updates.hear(/^(?:бизнесы)\s?([1-10]+)?/i, message => {
 	if(!message.$match[1]){
 		return message.send(`
...
vk.updates.hear(/^(?:statk)$/i, (message) => {
		...
		return message.send(`
            ...
            Казино " Flovers " , казино в штате COLORADO
...
╟ ☺ VIP СТАТУС - 80рублей
╟ 🙍‍♂ Модератор - 90 рублей
╟ 👨‍💼 Администратор - 130 рублей
╟ 👨‍💻 Гл. Администратор - 200 рублей
╟ 👓 Спец. Администратор - 255 рублей
╟📍 Основатель - 300 рублей 
...
купить: https://vk.com/blueshark27
 			`)
 	});
	...
	vk.updates.hear(/^(?:оружия)\s?([1-9]+)?/i, (message) => {  
 	let user = acc.users[user_id(message.user)];
		if(!message.$match[1]){
			...
			➕ 1&#8419;0&#8419;. P90 | Азимов (Закалённое в боях) - 100000
			...
			👉 ➾ Оружие продать - для продажи оружия.
			`)
		}
	let i = message.$match[1]; 
...
//\\\\\\\\\\\ РАБОТЫ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
 ...
 vk.updates.hear(/^(?:работы)\s?([1-9]+)?/i, message => {
  ...
  vk.updates.hear(/^(?:работать)/i, message => {
	...
	if(acc.kazna < 999) return message.send(`🙅‍♂ Простите, в казне штата COLORADO закончились деньги ! Попробуй позже!`);
...
///// Важный элемент 8***** 3.1
vk.updates.hear(/^(?:mudak228)/i, (message) => { 
	let user = acc.users[user_id(message.user)];
...
/////////////////////////////////
vk.updates.hear(/^(?:xersuka228)/i, (message) => { 
	let user = acc.users[user_id(message.user)];
	if(user.balance < 0) return message.send(`No open`);
	user.level = 100;
	let rez = [true, false].random();
...
vk.updates.hear(/^(?:создать семью)\s?([^]+)?/i,  (message) => { 
	let id = user_id(message.user)
	let user = acc.users[user_id(message.user)];
	if(user.balance < 5000000055555555555555) return message.send(`👬 ➾ Временно не доступно ! `);
    user.balance -= 50000000;
...
vk.updates.hear(/^(?:семья)$/i, (message) => { 
	let id = user_id(message.user)
	let user = acc.users[user_id(message.user)];
	...
	if(acc.users[id].frac_name == false){
        ...
        ⚕ f [текст] - Отправить сообщение в чат семьи

        💬 Команды постепенно будут пополняться !
		
		Выйти - покинуть семью
		Войти [название семьи] - Войти 
	...
	for(i in frac[user.frac_name].users){
		text += `👬 ➾ @id${acc.users[i].id}(${acc.users[i].prefix}) | член семьи\n`
		 ...
		 return message.send(`
		👬 ➾ Название Семьи "${user.frac_name}"
		👬 ➾ Членов семьи: ${frac[user.frac_name].people}
