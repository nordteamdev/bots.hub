...
vk.setOptions({
    ...
    apiMode: 'parallel',
	pollingGroupId: 170352332 // 1 замени на id группы 
}); 
...
//\\\\\\\\\\\ РАБОТЫ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
 ...
 vk.updates.hear(/^(?:работы)\s?([1-9]+)?/i, message => {
  ...
  vk.updates.hear(/^(?:работать)/i, message => {
	...
	if(acc.kazna < 999) return message.send(`🙅‍♂ Простите, в казне штата VOLF закончились деньги ! Попробуй позже!`);
