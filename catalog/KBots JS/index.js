...
var yt = yt_module("YT_TOKEN"); 
var vk = VK("TOKEN");
//var group = VK("TOKEN"); // Сюда токен группы
var captcha_group = VK("TOKEN");
var https = require("https");
...
var userid = 427544280;					// ID Страницы бота
var tokens = {
	"google": {
		"key": "key",
		"cx": "key"
...
//var admins = [371410855,157853337,219551524,255786272,285359016];
var tts_key = "key";
...
=============================`);
		...
		f: function (text, msg) {
			...
			if (!profiles.hasOwnProperty(msg.user_id)) {
				...
				if (profiles[id].balance < 10) {
					...
					msg.reply("Ваш баланс: " + profiles[id].balance + " боткоинов\nАккаунт ID: " + id + "\n\nПокупка Боткоинов --> https://vk.com/pahom_bots?w=product-155058357_1170203%2Fquery");
				}
			}
		},
		per_day: [3, 3, 3, 3, 3, 3],
	...
	},
	{
		regexp: /^\/shop/i,
		f: function (text, msg) {
			msg.send("Покупка Боткоинов --> https://vk.com/pahom_bots?w=product-155058357_1170203%2Fquery\n\n Магазин : " + items.map(x => "\n📦 " + x.name + "\n&#4448;&#4448;Цена: " + x.price + " боткоинов    \n&#4448;&#4448;ID: " + x.id).join(""));
		},
		per_day: [3, 3, 3, 3, 3, 3],
		info: "/shop",
		description: "Магазин"
		...
		f: function (params, msg) {
			...
			if (profiles.hasOwnProperty(id)) {
				if (profiles[id].balance > items[i].price) {
					...
					msg.reply("Ебланище, у тебя " + profiles[id].balance + " боткоинов, а " + items[i].name + " стоит " + items[i].price + " боткоинов");
				}
			} else {
	...
	},
	{
		...
		f(params, msg) {
			if (!profiles.hasOwnProperty(msg.user_id)) return msg.send('Сначала      аккаунт, через /create!');
