...
vk.setOptions({ token: '48564f6ef4401a5829b5bd9f933c7fc4cdf287b9b4f1be8ff2ae18b814c8ec3fde7ce196f4c252b57fa4f', pollingGroupId:177758581 });
const { updates, snippets } = vk;

updates.startPolling();
updates.on('message', async (message) => {
	if(Number(message.senderId) <= 0) return;
	if(/\[club177758581\|(.*)\]/i.test(message.text)) message.text = message.text.replace(/\[club177758581\|(.*)\]/ig, '').trim();
...
🔹Что бы приобрести рубли, добавьте в друзья [415546404|Артёма Арсентьева] с пометкой "rub"`);
});

cmd.on(/^(?:Аккаунты|Привилегии)$/i, async (message, bot) => {
	return message.send(`
💠Аккаунты: 
 1⃣Модератор     Аккаунт➖50₽ 
 2⃣Адмиристратор     Аккаунт➖100₽ 
 3⃣Создатель     Аккаунт➖200₽
...
cmd.on(/^(?:Аккаунт 1)$/i, async (message, args, bot) => {   	
	if(message.user.rub < 50) return message.send(`💠 » Чтобы купить привилегию Модератор нужно иметь 50 руб`);
    message.user.rub -= 50
    message.user.right = 1;
...
cmd.on(/^(?:Аккаунт 2)$/i, async (message, args, bot) => {   	
	if(message.user.rub < 100) return message.send(`💠 » Чтобы купить привилегию Администратор нужно иметь 100 руб`);
...
🔹Что бы купить аккаунт, введите "Валюта [номер]"
🔹Что бы приобрести рубли, добавьте в друзья [415546404|Артёма Арсентьева] с пометкой "rub"
...
message.send(`ID чата: ${message.chat}`)
});
...
cmd.on(/^(?:казино)\s(.*)$/i, async (message, bot) => {
	...
	{
		message.user.balance -= message.args[1];
		const multiply = utils.pick([2, 0.75, 0.75, 0, 2, 0.75, 0.75, 2, 0.5, 0.5, 0, 0.5, 2, 2, 0.5, 0.50, 2, 0, 0.75, 2, 1, 10, 1, 1, 0.5, 9, 2, 10, 2, 1, 1, 2, 1, 1, 2, 2, 0.75, 5, 2, 0, 20, 0, 0.5, 2, 0.5, 0.5, 6, 2, 2, 0, 2, 1, 1, 1, 1, 0.5, 0.5, 23]);
