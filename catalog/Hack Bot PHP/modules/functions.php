...
function ChangeName(){
	...
	$pricedonate = 1;
		...
		$BlackArr = array ('.','[',']','(',')','|','@','riconc','бог','адм','разраб','adm','создатель','творец','риконс','✓ | [Администрация]');
		$nicknamecheck = str_replace($BlackArr,'',substr($bodyl,7),$countss);
		if($countss>0){
			$nickname = str_replace($BlackArr,'жооооопа',substr($bodyl,7));
			$nickname = SetNickName($nickname);
			SetFieldF('name',$nickname);
		}else{
			$nickname = str_replace($BlackArr,'жооооопа',substr($body,7));
			$nickname = SetNickName($nickname);
			SetFieldF('name',$nickname);
		}
