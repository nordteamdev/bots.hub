...
{    
    ...
    public function SearchVariables($text){
        if(!empty($text)){
            $arraySearch = array('x','y','z','a','b');
            for($i=0;$i<count($arraySearch);$i++){
                $symbol_pos = strpos($text,$arraySearch[$i]);

                if(stristr($text,$arraySearch[$i])){
                    $otrez_sym = substr($text, $symbol_pos);
                   
                    $array = array("/".$arraySearch[$i]."/","/=/");
                    if(stristr($otrez_sym,',')){
                        $otrez = substr($otrez_sym, 0, strpos($otrez_sym,','));
                    }else{
                        $otrez = $otrez_sym;
                    }
                    
                
                    $value_var = preg_replace($array, '', $otrez);         
                    $arrayVariables[$arraySearch[$i]] = $value_var;
