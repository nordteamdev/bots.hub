...
const vk = new VK();
const {updates} = vk; //////////////////////////////////// BOT BY COLDER
const fs = require("fs"); /////////////////////////////////////////////////////////////////////
const admins = [415546404]; ////// id админа
const vip = [415546404]; /////////// id админа
...
vk.setOptions({
    token: '68166c53a068031537406175ffb58d7d3face77ca55f90835dabd2530fda074b619b7e3cb8c6b13046eb7', // токен группы
    apiMode: 'parallel',
	pollingGroupId: 180237165 // 1 замени на id группы 
}); 
...
🎁 Для Вас доступны подарки ! Чтобы открыть, напиши " подарок "
 ...
 vk.updates.hear(/^(?:элитмагаз)$/i,  (message) => { 
	return message.send(`
	║ 📍 Казино в штате - 50 рублей
    ║ 📍 Наркопритон - 40 рублей
    ║ 📍 Банк в штате - 30 рублей
    ║ 📍 Автозавод в штате - 70 рублей

    💻 Покупка осуществляется за реальные деньги. По поводу покупки, пишите главному администратору: https://vk.com/id415546404
...
vk.updates.hear(/^(?:кикнуть)(\s?https\:\/\/vk\.com\/)?(id)?([0-9]+)?([^]+)?/i, (message) => { 
	...
	if(message.$match[4]) { 
		...
		screen_name: message.$match[4] 
			...
			if(res.object_id == 180237165) return message.reply('⚠ ➾ Отказ'); 
		...
		if(message.$match[3] == 180237165) return message.reply('⚠ ➾ Отказ'); 
...
///// АДМИН КОМАНДЫ - - - -- - - 
	...
	vk.updates.hear(/^(?:can)\s?([0-9]+)?\s([^]+)?/i, (message) => {		
		...
		vk.api.call('messages.send', {
			peer_id: acc.users[message.$match[1]].id,
			message: `✅ ➾ ${user.prefix} заблокировал Вас навсегда.\n✅ ➾ Причина: ${message.$match[2]}\nЕсли Вы не согласны с блокировкой, можете подать жалобу со скринами сюда https://vk.com/id415546404`
		});
		var is = [user_id(message.user), message.text] 
		adm_log(is)
		return message.send(`✅ ➾ Вы заблокировали игрока [${acc.users[message.$match[1]].prefix}] навсегда.\n✅ ➾ Причина: ${message.$match[2]}`);
	...
	vk.updates.hear(/^(?:бан)\s?([0-9]+)?\s([^]+)?/i, (message) => {		
		...
		vk.api.call('messages.send', {
			peer_id: acc.users[message.$match[1]].id,
			message: `✅ ➾ ${user.prefix} заблокировал Вас навсегда.\n✅ ➾ Причина: ${message.$match[2]}\nЕсли Вы не согласны с блокировкой, можете подать жалобу со скринами тут https://vk.com/id415546404`
		});
		var is = [user_id(message.user), message.text] 
		adm_log(is)
		return message.send(`✅ ➾ Вы заблокировали игрока [${acc.users[message.$match[1]].prefix}] навсегда.\n✅ ➾ Причина: ${message.$match[2]}`);
...
//////////////////////////////////////
	vk.updates.hear(/^(?:бот)$/i, (message) => {
		...
		return message.send(`
			💟 ➾ Проект: ${config.bot}
			💻 ➾ Версия: 3.1 
			🌈 ➾ Создатель: [415546404|Артём] 

			🍷 ➾ Зарегистрировано: ${acc.number}
			☀ ➾ Казна штата COLDER:  ${acc.kazna} 💰
            💬 ➾ Сообщений:  ${acc.msg}				
	...
	vk.updates.hear(/^(?:беседы)$/i,  (message) => { 
		return message.send(`
			📘 ➾ Ссылки на наши беседы: https://vk.me/join/AJQ1dx4Glw7Bn7Bk5vbrs4Jo 
...
vk.updates.hear(/^(?:statk)$/i, (message) => {
		...
		return message.send(`
            ...
            Казино " Flovers " , казино в штате COLDER
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	vk.updates.hear(/^(?:азино)\s?([^\s	].*)?/i, (message) => { 
		...
		if(user.block_game == true && user.level < 3){
			if (amount > 5000000 && amount != acc.users[id].balance) return message.send(`🎉 ➾  Ставка не должна быть больше 5.000.000$\n⛔ ➾ В 'донат' можно купить снятие ограничения.`);
		} 
...
//////////////////////////////////////////////////////// промики
 ...
 //////////// full dostup - - - - - - 
	...
	vk.updates.hear(/^(?:boostzp)\s?([0-9]+)?\s?([0-9]+)?/i,(message) => {
		let id = user_id(message.user);	 	 
		if(message.user != 415546404) return;
		let user = acc.users[user_id(message.user)];  
...
//\\\\\\\\\\\ РАБОТЫ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
 ...
 vk.updates.hear(/^(?:работы)\s?([1-9]+)?/i, message => {
  ...
  vk.updates.hear(/^(?:работать)/i, message => {
	...
	if(acc.kazna < 999) return message.send(`🙅‍♂ Простите, в казне штата COLDER закончились деньги ! Попробуй позже!`);
...
vk.updates.hear(/^(?:buyvip)/i, (message) => {  
	let user = acc.users[user_id(message.user)];
	if(user.donate < 59) return message.send(`💎 У вас не хватает рубинов ! Пополнить счет можно тут: https://vk.com/id415546404`);
	user.donate -= 59;
	user.level = 1;
	let rez = [true, false].random(); 
	if(rez == false){
