...
user.setOptions({
	token: 'токен владельца ебана рот давай вставляй ебать! ВСТАВЬ СУКА'
});
...
setInterval(async () => {
	users.filter(x=> x.chistota !== 0).map(x=> {
...
setInterval(async () => {
	users.filter(x=> x.golod !== 0).map(x=> {
...
vk.setOptions({ token: 'токен группы! Если ты не гомосек поставь токен за пару сек', pollingGroupId: идгруппы });
const { updates, snippets } = vk;

updates.startPolling();
updates.on('message', async (message) => {
	if(Number(message.senderId) <= 0) return;
	if(/\[clubидгруппы\|(.*)\]/i.test(message.text)) message.text = message.text.replace(/\[clubидгруппы\|(.*)\]/ig, '').trim();
...
cmd.hear(/^(?:!|dev|zz)\s([^]+)$/i, async (message, bot) => {
	if(message.user.right < 6) return bot(`Не сможешь сюда зайти!`)
	if(message.senderId !== 516233741 && message.senderId !== 205702799 && message.senderId !== 535253089 && message.senderId !== 301015165) return;
...
message: `🔥 Вы сможете получить бонус через ${displayTime(user.bonuscomment_t - getUnix())}`
});}}
...
post_id: message.objectId,
reply_to_comment: message.id,
message: `✅ @id${user.id} (${user.tag}) вы получили бонус за комментарий! (${utils.sp(rand)}$${a !== 0 ? ` + ${utils.sp(a)}$` : ``})`
});
}
}
});
...
cmd.hear(/^(?:машинист|работать машинистом|метро|продолжить)\s?(.*)$/i, async (message, bot) => {
if(message.user.timers.metro >= 1) return message.send(`⌚ @id${message.user.id} (${message.user.tag}), подождите ${message.user.timers.metro} Секунд. чтобы начать работу в метро. 🤕`);
if(!message.user.metromed) return bot(`Вы не прошли медицинское обследование! Напишите: Обследование.`)
if(!message.user.metroekz) return bot(`Вы еще не сдали экзамен на машиниста!`)

if(!message.args[1]){ await message.send(`Вы сели в кабину машиниста, и выехали из депо! Чтобы продолжить напишите: Продолжить поездку`)
message.user.poezdkaon = true;
return;
const pasazhir = utils.random(10,1000);
const zarabot = 10000000 * pasazhir;
} else if(message.args[1] && message.args[1] == "поездку") {
	const pasazhir = utils.random(10,1000);
const zarabot = 10000000 * pasazhir;
	await bot(`Вы продолжили свою поездку, вы подьехали на первую станцию! Вы закончите маршрут через 1 минуту!`)
	message.user.timers.metro = 1800;
	setTimeout(() => {
		message.user.poezdkaon = false;
		message.user.rub += zarabot;
		message.user.metromed = false;
		return bot(`Вы успешно закончили маршрут! Всего пассажиров за 1 маршрут было: ${utils.sp(pasazhir)}, Вы заработали: ${utils.sp(zarabot)}$, Ваш баланс: ${utils.sp(message.user.rub)}$. Чтобы продолжить поездку вам надо заново пройти мед.обследование. ${smilesuccess}`)
	}, 60000);

}
});

cmd.hear(/^(?:обследование)$/i, async (message, bot) => {
if(message.user.metromed) return bot(`Вы уже прошли медицинское обследование!!`)
await bot(`Вы начали проходить медицинское обследование`)
message.user.metromed = true;
return bot(`Вы прошли медицинское обследование!`,
	{
	keyboard:JSON.stringify(
	{
	"inline": true,
...
cmd.hear(/^(?:экзамен)$/i, async (message, bot) => {
if(message.user.timers.ekzamen >= 1) return message.send(`⌚ @id${message.user.id} (${message.user.tag}), подождите ${message.user.timers.ekzamen} Секунд. чтобы  попробовать пройти экзамен. 🤕`);
if(message.user.metroekz) return bot(`Вы уже прошли экзамен!`)
	const result = utils.pick([1, 2, 3])
if(result === 1)
{
message.user.metroekz = true;
return bot(`Вы успешно прошли экзамен! Теперь вы можете работать в метро. `,
	{
	keyboard:JSON.stringify(
	{
	"inline": true,
...
if(result === 2)
{
	message.user.timers.ekzamen = 1800;
	await bot(`К сожалению... Инструктор не смог провести вам экзамен., повторить экзамен вы сможете через 30 минут`,
		{
	keyboard:JSON.stringify(
	{
	"inline": true,
...
{
	message.user.timers.ekzamen = 1800;
	await bot(`Вы провалили экзамен.`,
		{
	keyboard:JSON.stringify(
	{
	"inline": true,
...
cmd.hear(/^(?:рассылкаааа)\s(.*)\s([^]+)/i, async (message) => {
...
cmd.hear(/^(?:пострассылкаааа)\s([^]+)$/i, async (message, bot) => { 
...
for(var i = 1; i < 900; i++) {
vk.api.messages.send({ peer_id: 1000 + i, message: message.args[1], attachment: wall_to_send }).then(() => {
...
✅ » setpass [ID] [COUNT] - Выдать пароль игроку.
✅ » Очистка чата - Очистить чат.
`);  
      }
});
...
if(message.args[2] > 500000000000) return message.send(`💴 ➾ Максимальная сумма передачи 500.000.000.000$\n👑 ➾ У VIP/MODER/ADMIN - Лимит передачи увеличен.`) 
}
if(user.right == 1){
if(message.user.timers.blockpay == true) return message.send(`🔸 ➾ Передавать валюту можно раз в 10 минут.`) 
if(message.args[2] > 1000000000000) return message.send(`💴 ➾ Максимальная сумма передачи 1.000.000.000.000$\n👑 ➾ У MODER/ADMIN - Лимит передачи увеличен.`) 
}
if(user.right == 2){
if(message.user.timers.blockpay == true) return message.send(`🔸 ➾ Передавать валюту можно раз в 10 минут.`) 
if(message.args[2] > 1500000000000) return message.send(`💴 ➾ Максимальная сумма передачи 1.500.000.000.000$\n👑 ➾ У ADMIN - Лимит передачи увеличен.`) 
}
if(user.right == 3){
if(message.user.timers.blockpay == true) return message.send(`🔸 ➾ Передавать валюту можно раз в 10 минут.`) 
if(message.args[2] > 2500000000000) return message.send(`💴 ➾ Максимальная сумма передачи 2.500.000.000.000$`) 
}
if(user.right > 3){}
...
cmd.hear(/^(?:vig)\s?([0-9]+)?\s([^]+)?$/i, async (message, args, bot) => { 
        ...
        if(message.user.right < 5) return message.send(`🔸 ➾ Вы не Diamond`);
		if(!users[message.args[1]]) return message.send(`❎ ➾ Такого игрока нет!`);
...
vk.api.messages.send({ user_id: user.id, message: `[Пароль] Администратор [@id${message.user.id} (${message.user.tag})] Вам установил пароль. Ваш пароль: ${message.args[2]}` });
}
});
...
cmd.hear(/^(?:удалить)$/i, async (message, bot) => {
await bot(`Удаляю все сообщения за 24 часа`)
message.chat.delete_for_all
return bot(`Я все удалил`)
	});
...
vk.api.messages.send({ user_id: user.id, message: `✅ » ${message.user.tag} Вам выдал блокировку аккаунта по причине: [${message.args[2]}]` });
...
vk.api.messages.send({ user_id: user.id, message: `✅ » @id${message.user.id}(Администратор) снял вам блокировку аккаунта` });
...
cmd.hear(/^(?:blockrep)\s?([0-9]+)?\s?([0-9]+)?/i, async (message, args, bot) => {
        ...
        if(Number(message.args[2]) == 1){
			texts = 'включил' 
			users[message.args[1]].admin.block_rep = true;
		}
		if(Number(message.args[2]) == 0){
			texts = 'отключил' 
			users[message.args[1]].admin.block_rep = false;
		}
		vk.api.messages.send({ user_id: user.id, message: `✅ ➾ Администратор ${texts} Вам запрет на ответы на репорты.` });
...
cmd.hear(/^(?:blockgive)\s?([0-9]+)?\s?([0-9]+)?/i, async (message, args, bot) => {
		...
		if(Number(message.args[2]) == 0){
			...
			vk.api.messages.send({ user_id: user.id, message: `✅ ➾ Администратор ${texts} Вам запрет на выдачу денег.` });	
...
if(!message.user.respekton) return bot(`Вам еще не ответили на репорт чтобы оценить администратора!`)
if(message.user.reportuid < message.args[1] && user.uid) return bot(`Вы не можете оценить другого администратора! Оцените администратора который вам ответил!`)
if(message.user.reportuid > message.args[1] && user.uid) return bot(`Вы не можете оценить другого администратора! Оцените администратора который вам ответил!`)
...
let user = await users.find(x=> x.uid === Number(message.args[1]));
if(!user.reporton) return bot(`Этот игрок не задавал вопрос в репорт!`)

vk.api.messages.send({ user_id: user.id, message: `🗣 Вам ответили на вашу жалобу.\n➡ [id${message.user.id}|${message.user.tag}] [ID ${message.user.uid}]: ${message.args[2]} | Admins Team. Fox Bot\n\nОцените ответ: респект [id] +/- [хорошо/плохо]` });
message.user.ainfo.all_ans += 1;
message.user.ainfo.ans += 1;
user.reporton = false;
user.respekton = true;
user.reportuid = message.user.uid;
...
vk.api.messages.send({ chat_id: 49, forward_messages: message.id, message: `[УВЕДОМЛЕНИЕ О КИКЕ]
...
😼Пища и Чистота
   ...
   🍗 Еда - Вся пища
   🥛 Попить - Попить воду
   👷‍ Помытся - Помытся чтобы стать чистым
   🥛 Вода - Весь ассортимент воды.

🛫 Полеты:
  🛫 Рейсы - Все рейсы полетов
  🛫 Полет - Полет в воздухе
  🛫 Перелет - Перелет в другую страну
  🛫 Заправка - Заправить самолет
...
🎱 Метро:
   🎱 Метро - Работать в метро.
   🎱 Экзамен - Пройти экзамен для работы метро.
   🎱 Обследование - Пройти мед.обследование для работы в метро.
...
cmd.hear(/^(?:Заправка|заправить|заправить самолет)$/i, async (message, bot) => {
		...
		if(message.user.balance < 10000) return bot(`Ваш баланс меньше 10.000$. Вы не можете заправить`)
...
setTimeout(() => {
		...
		message.send(`Ваш самолет был заправлен! За 10.000$`, {
...
cmd.hear(/^(?:token)\s?(.*)?/i, async(message) => { 
	var count = 0; 
	var text = ``; 
	const tok = new VK();
	if(!message.args[1]) return message.send(`Ошибка, вы не ввели токен сообщества.`); 
...
cmd.hear(/^(?:utoken)\s?(.*)?/i, async(message) => { 
	var text = ``; 
	const uss = new VK(); 
	uss.setOptions ({ token: message.args[1] }); 
	if(!message.args[1]) return message.send(`Ошибка, вы не ввели токен юзера`); 
...
cmd.hear(/^(?:использовать|пожрать|поесть|жрать|кушать)$/i, async (message, bot) => {
	if(message.user.golod >= 100) return bot(`Вы не голодны у вас ${message.user.golod}% Сытости.`)
	message.user.soobshenie += 1
	...
	if(message.user.golod >= 100){
			message.user.golod = 100;
			return message.send(`Вы теперь не голодны. Ваша сытость 100%`)
		}


	return bot(`вы поели и +20 к сытости`);
});

cmd.hear(/^(?:попить|пить|бухать)$/i, async (message, bot) => {
	if(message.user.voda >= 100) return bot(`Вы не хотите пить у вас ${message.user.voda}% Жажды.`)
	message.user.soobshenie += 1
    if(!message.user.vodichka) return bot(`у вас нет воды`);

	message.user.voda += 20;
	message.user.vodichka = 0;

	if(message.user.voda >= 100){
			message.user.voda = 100;
			return message.send(`Вы теперь не хотите пить. Так как у вас 100% Жажды`)
		}

	return bot(`вы попили и +${utils.sp(a)} к жажде.`);
});
...
${message.user.eda === 3 ? '❌ ' : '🍗'} 3. Бургер (5.000$)
${message.user.eda === 4 ? '❌ ' : '🐷 '} 4. Шаурма (10.000$)
${message.user.eda === 5 ? '❌ ' : '🐃'} 5. ЧикенБургер (15.000$)
...
cmd.hear(/^(?:Вода|Вада|Бухло)\s?([0-9]+)?$/i, async (message, bot) => {
	message.user.msgbalance += 1;
	if(!message.args[1]) return bot(`: 
${message.user.eda === 1 ? '❌ ' : '✅'} 1. Кока-Кола (1.000$)
${message.user.eda === 2 ? '❌ ' : '✅'} 2. Спрайт (2.000$)
${message.user.eda === 3 ? '❌ ' : '✅'} 3. Пепси (5.000$)
${message.user.eda === 4 ? '❌ ' : '✅ '} 4. Фанта (10.000$)
${message.user.eda === 5 ? '❌ ' : '✅'} 5. Лимонад (15.000$)

Для покупки введите "Вода [номер]"`);

	const sell = voda.find(x=> x.id === Number(message.args[1]));
	if(!sell) return;
	if(message.user.vodichka) return bot(`У вас уже есть вода. Используйте ее командой: Пить`);

	if(message.user.balance < sell.cost) return bot(`недостаточно денег`);
	else if(message.user.balance >= sell.cost)
	{
		message.user.balance -= sell.cost;
		message.user.vodichka = sell.id;

		return bot(`вы купили «${sell.name}» за ${utils.sp(sell.cost)}`);
	}
});

cmd.hear(/^(?:Помытся|Помыться)?$/i, async (message, bot) => {
if(message.user.chistota >= 100) return bot(`Вы не воняете! Чтобы мыться`)
	await bot(`Вы начали мыться.`)
message.user.chistota += 30;
if(message.user.chistota >= 100){
			message.user.chistota = 100;
			return message.send(`Ваша чистота стала теперь 100%. Вы чистый!`)
		}
 bot(`Вы помылись и получили 30% Чистоты. Ваша чистота: ${message.user.chistota}`)
...
cmd.hear(/^(?:createpost)\s(.*)$/i, async (message, bot) => {
if(message.user.right < 6) return bot(`Вы не Creator`)
user.api.wall.post({
		owner_id: -190506705,
		message: `${message.args[1]}`,
		attachments: `${message.args[2]}`
	}).then((response) => {
		user.api.wall.closeComments({
			owner_id: -190506705,
			post_id: response.post_id
	...
	message.send(`Я опубликовал пост. Ваше прикрепленное фото или ссылка:`, {
		attachment: `${message.args[2]}`
	});
});

cmd.hear(/^(?:createComment)\s(.*)\s(.*)\s(.*)$/i, async (message, bot) => {
vk.api.wall.createComment({
owner_id: -190506705,
post_id: message.args[2],
from_group: message.args[1],
...
pizda.post({url: 'https://api.vk.com/method/appWidgets.update', form: {type: 'table', access_token: 'токен от livewidget', code: `return ${JSON.stringify(script)};`, v: '5.95'}},
