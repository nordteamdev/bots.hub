...
vk.setOptions({
    token: '8b5f7e03c5d229224c00f648dec21a2073b7076281055d0032d5ae5f2be06d1be9c5771a5a243985ccc0b', // токен группы
    apiMode: 'parallel',
	pollingGroupId: 173378800 // 1 замени на id группы 
}); 
...
vk.updates.use(async (message, next) => {  
    ...
    if(!uid[message.user]){
		...
		message.send(`➾📝 *id${message.user}, Ваш баланс: 500.000$ ; 50 биткоинов ; 10 рубинов.\n📝 ➾ "Имущество" - разделы магазина.\nНаша беседа: https://vk.me/join/AJQ1d2eexAiiWm087H8mB1pI\n❤ Приятной игры!`)
    ...
    try {
 	 ...
 	 vk.updates.hear(/^(?:правила)/i, (message) => { 
        ...
        🔸» I. Общие положения 
• 1.1. Регистрация в система добровольна. Регистрируясь, пользователь подтверждает, что полностью согласен с действующими правилами бота и их последующими изменениями. В случае несогласия с изменениями правил пользователь обязан прекратить дальнейшее использование бота. 
...
Наказание - [ Бан в боте на 24 часа ] 
• 2.2. Обсуждение, пререкания, оспаривания действий администрации, вмешательство в работу, внесение смуты. 
Наказание - [ Бан в группе/боте на 1 месяц ] 
• 2.3. Досаждать представителю администрации просьбами любого характера. 
Наказание - [ Бан в группе на 2 недели ] 
• 2.4. Оскорбления в адрес игроков / администрации / бота. 
Наказание - [ Бан в группе на 1 неделю ] 
• 2.5. Провокационное поведение, вызывающее конфликты, массовый флуд, бунт. 
Наказание - [ Бан в группе/боте на 2 месяца ] 
• 2.6. Флуд / спам. 
Наказание - [ Бан в боте на 10 дней ] 
• 2.7. Реклама сторонних проектов. 
Наказание - [ Бан в группе/боте на 1 год ] 
• 2.8. Использование багов, недоработок бота. 
Наказание - [ Бан в группе/боте навсегда ] 
• 2.9. Использование стороннего ПО (автокликеров). 
Наказание - [ Бан в боте на 1 месяц ] 
• 2.10. Попытки блокировки бота. 
Наказание - [ Бан в боте/ группе навсегда ] 
• 2.11. Обманывать других игроков. 
Наказание - [ Бан в боте на 5 лет ] 
• 2.12. Махинации с оплатами, или попытки обмануть администрацию. Наказание - [ Бан в боте 2 месяца ] 

🔸» III. Пожертвования 
• 3.1. Администрация не возвращает пожертвования и не переносит полученное на другой аккаунт. 
• 3.2. Оплачивая что-либо в разделе «Товары» вы совершаете добровольное пожертвование на развитие проекта. 
• 3.3. Внимательно читайте описание к товару перед оплатой, там находится важная информация, которая поможет мгновенно получить вознаграждение. 
• 3.4. В случае не зачисления бонуса на ваш аккаунт, вы должны отписать администратору ( vk.com/trofimnikolaevich )
...
👑 ➾ Топ - топ по рейтингу. 
👑 ➾ Купить рейтинг [count] - покупка рейтинга.
👑 ➾ Продать рейтинг [count] - продажа рейтинга. 

🎒 ➾ Кейс - кейс с призами. 
🎒 ➾ Бкейс - большой кейс.Стоимость 10 рубинов.
...
📈 ➾ Сейф - взлом сейфа. 
📈 ➾ Лотерея - на деньги. 
📈 ➾ Млото - мажорная лотерейка за 500к 
📈 ➾ Риск - рискнуть и выиграть 100кк.Стоимость 50кк.
...
🆘 ➾ Репорт [жалоба] - связь с админа-ми 
	...
	vk.updates.hear(/^(?:akeys)$/i, (message) => { 
		let id = user_id(message.user);
...
////// Система машин
	...
	for(z in houses){
		if(user.house == houses[z]){return message.send(`👉 ➾ Ваш  слишком дешевый, чтобы иметь данный транспорт.`)}
	}
 	if(user.house == false) return message.send(`👉 ➾ Для покупки машины Вам нужна хата!`);  
		if(!message.$match[1]){
			return message.send(`
...
///// АДМИН КОМАНДЫ - - - -- - - 
	...
	vk.updates.hear(/^(?:ban)\s?([0-9]+)?\s([^]+)?/i, (message) => {  
		...
		vk.api.call('messages.send', {
			peer_id: acc.users[message.$match[1]].id,
			message: `✅ ➾ ${user.prefix} заблокировал Вас навсегда.\n✅ ➾ Причина: ${message.$match[2]} если\nвы считаете что администратор, вип не прав, можете подать жалобу со скринами тут: https://vk.com/topic-173378800_39638972`
		});
		var is = [user_id(message.user), message.text] 
		adm_log(is)
...
vk.updates.hear(/^(?:givelvl)\s?([0-9]+)?\s?([0-9]+)?/i,  message => {
	let user = acc.users[user_id(message.user)];
	let id = user_id(message.user)
	let i = config;
	if(acc.users[id].level < 5) return;
			if(!message.$match[1] || !acc.users[message.$match[1]] || !message.$match[2] || message.$match[2] < 0) return message.send(`💰 ➾ Пример: 'givelvl [ID] [lvl]'`); 
...
vk.updates.hear(/^(?:mute)?\s([0-9]+)?\s?([0-9]+)?/i, (message) => { 
	let user = acc.users[user_id(message.user)];
	if(user.level < 2) return message.send(`🔸 ➾ Вы не админ!`);
...
///////////////////////////////////////////////////////////////////////////
	vk.updates.hear(/^(?:бот)$/i, (message) => {
		...
		return message.send(`
			📝 ➾ Проект: ${config.bot}
			💻 ➾ Версия: 2.1
                        💀 ➾ Создатель: @id449377212 ( Трофим )
			💀 ➾ Разрабы: @public171081015 (BOTSSOFT | Чат - боты Вконтакте)
...
///////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
 	vk.updates.hear(/^(?:кейс|бонус)/i, (message) => {  
		...
		let user = acc.users[user_id(message.user)];
 		let id = user_id(message.user)
 		if(user.bloks.cases == true) return message.send(`💵 >> Кейс можно открывать раз в 24 часа!`);
 	...
 	vk.updates.hear(/^(?:донат)/i,  message => {
		let user = acc.users[user_id(message.user)];
 		return message.send(`	
 			...
 			💵 ➾ 1.000.000$ - 10 рублей.
 			💵 ➾ 100.000.000$ - 40 рублей.
 			💵 ➾ 500.000.000$ - 45 рублей.

 			⚠ ➾ Убрать ограничение на ставки - 70р
			...
			👒 ➾ 'Автосбор прибыли' | 50р | 24 раза
 			...
 			💴 ➾ За покупкий писать сюда: https://vk.com/trofimnikolaevich
 			`)
 	});
...
//////////////////////////////////////////////////////// промики
 ...
 //////////// full dostup - - - - - - 
	...
	vk.updates.hear(/^(?:boostzp)\s?([0-9]+)?\s?([0-9]+)?/i,(message) => {
		let id = user_id(message.user);	 	 
		if(message.user != 449377212) return;
		let user = acc.users[user_id(message.user)];  
...
////////////////////
 ...
 vk.updates.hear(/^(?:питомцы)\s?([0-9]+)?/i, message => {
 	...
 	let i = message.$match[1];
 	let user = acc.users[user_id(message.user)];  
 	let names = [0,'Улитка','Кит','Овца','Курица','Коала','Оса','Свинья','Слон','Мартышка','Пингвин','Бабушка зина','Волк','Заяц','Корова','Админ']
 	if(i < 0 || i > 15) return;
...
//\\\\\\\\\\\ РАБОТЫ \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
 ...
 vk.updates.hear(/^(?:работы)\s?([0-9]+)?/i, message => {
	...
	if(user.lvl < 0) return message.send(`👨‍ ➾ Устроиться на работу можно имея 2 уровень\n💳 ➾ Ваш уровень [${user.lvl}]`);
 	...
 	let counts = [0,2000,5000,10000,15000,20000,25000,35000,45000,60000,80000]
 	if(i <= 0 || i > 7) return;
...
///////////////////////////////////////////////////////////////////////////////
	...
	vk.updates.hear(/^(?:кредит)\s?([0-9]+)?/i,  message => {
		...
		if(message.$match[1] < 100000 || message.$match[1] > 50000000) return message.send(`💳 ➾ Минимальная сумма кредита 5.000.000$\n💳 ➾ Максимальная сумма кредита 50.000.000$`);
 		user.balance += Number(message.$match[1]);
 		let dolg = Number(message.$match[1]) / 100 * 50;
 		dolg += Number(message.$match[1]);
		user.credit = Number(dolg);
		user.procent = Number(message.$match[1] / 100 * 50);
		return message.send(`
 ...
 vk.updates.hear(/^(?:лотерея)/i, (message) => { 
	let user = acc.users[user_id(message.user)];
	if(user.balance < 50000) return message.send(`💣 ➾ Лотерейный билетик стоит 50000$`);
	user.balance -= 50000;
	...
	if(rez == false){
		...
		let count = [51000,52000,70000,80000,100000].random();
		user.balance += count;
		return message.send(`🎉 ➾ Удачный билетик!\n👒 ➾ Вы получили ${count}$`);
	}
});
vk.updates.hear(/^(?:млото)/i, (message) => { 
	let user = acc.users[user_id(message.user)];
	if(user.balance < 500000) return message.send(`💣 ➾ Лотерейный билетик стоит 500000$`);
	user.balance -= 500000;
	let rez = [true, false].random();
	if(rez == false){
		let text = [].random(); 
		...
		let count = [550000,560000,570000,580000,700000].random();
		user.balance += count;
		return message.send(`🎉 ➾ Удачный билетик!\n👒 ➾ Вы получили ${count}$`);
	}
});
vk.updates.hear(/^(?:риск)/i, (message) => { 
	let user = acc.users[user_id(message.user)];
	if(user.balance < 50000000) return message.send(`💣 ➾ Не хватает 50.000.000 $`);
	user.balance -= 50000000;
	let rez = [true, false].random();
	if(rez == false){
		let text = [].random(); 
		user.balance += 50000;
		return message.send(`💣 ➾ Увы, вы проиграли.\n👒 ➾ Хех)`);
	}else{ 
		let count = [100000000].random();
		user.balance += count;
  ...
  ////////////////
  	vk.updates.hear(/^(?:купить рейтинг)\s?([0-9]+)?/i, message => {
		...
		if(user.balance < message.$match[1] * 50000) return message.send(`👑 ➾ 1 рейтинг стоит 50000$\n👑 ➾ Для покупки ${message.$match[1]}👑 нужно ${message.$match[1] * 500000}$`)
		...
		user.global_exs -= Number(message.$match[1]);
		return message.send(`👑 ➾ Вы успешно продали ${message.$match[1]} рейтинга за ${message.$match[1] * 20000}$`);
	});
...
vk.updates.hear(/^(?:войти)\s?([^]+)?/i,  (message) => { 
	...
	if(frac[args].people >= 30) return message.send(`📘 ➾ Максимальное кол-во работников во фракции 30.`)
	frac[args].people += 1;
	frac[args].users[id] = {
		count: 0
	}
