...
! function ($) {
    ...
    if (void 0 === parallable) {
        var parallable = function (file, funct) {
            "use strict";
            return parallable.core[funct.toString()] = funct()
                .core
                , function () {
            ...
            function pre() {
                ...
                pyr[0] = canvas, ret[0] = {
                    ...
                    , data: pyr[0].getContext("2d")
                        .getImageData(0, 0, pyr[0].width, pyr[0].height)
                        .data
                };
                var i;
                for (i = 1; interval >= i; i++) pyr[4 * i] = document.createElement("canvas"), pyr[4 * i].width = Math.floor(pyr[0].width / Math.pow(scale, i))
                    , pyr[4 * i].height = Math.floor(pyr[0].height / Math.pow(scale, i)), pyr[4 * i].getContext("2d")
                    .drawImage(pyr[0], 0, 0, pyr[0].width, pyr[0].height, 0, 0, pyr[4 * i].width, pyr[4 * i].height)
                    , ret[4 * i] = {
                        width: pyr[4 * i].width
                ...
                for (i = next; scale_upto + 2 * next > i; i++) pyr[4 * i] = document.createElement("canvas")
                    ...
                    .drawImage(pyr[4 * i - 4 * next], 0, 0, pyr[4 * i - 4 * next].width, pyr[4 * i - 4 * next].height, 0, 0, pyr[4 * i].width, pyr[4 * i].height)
                    , ret[4 * i] = {
                        width: pyr[4 * i].width
                        , height: pyr[4 * i].height
                        , data: pyr[4 * i].getContext("2d")
                ...
                for (i = 2 * next; scale_upto + 2 * next > i; i++) pyr[4 * i + 1] = document.createElement("canvas")
                    ...
                    .drawImage(pyr[4 * i - 4 * next], 1, 0, pyr[4 * i - 4 * next].width - 1, pyr[4 * i - 4 * next].height, 0, 0, pyr[4 * i + 1].width - 2, pyr[4 * i + 1].height)
                    , ret[4 * i + 1] = {
                    ...
                    , pyr[4 * i + 2].height = Math.floor(pyr[4 * i - 4 * next].height / 2), pyr[4 * i + 2].getContext("2d")
                    .drawImage(pyr[4 * i - 4 * next], 0, 1, pyr[4 * i - 4 * next].width, pyr[4 * i - 4 * next].height - 1, 0, 0, pyr[4 * i + 2].width, pyr[4 * i + 2].height - 2)
                    , ret[4 * i + 2] = {
                    ...
                    , pyr[4 * i + 3].height = Math.floor(pyr[4 * i - 4 * next].height / 2), pyr[4 * i + 3].getContext("2d")
                    .drawImage(pyr[4 * i - 4 * next], 1, 1, pyr[4 * i - 4 * next].width - 1, pyr[4 * i - 4 * next].height - 1, 0, 0, pyr[4 * i + 3].width - 2, pyr[4 * i + 3].height - 2)
                    , ret[4 * i + 3] = {
...
fdt_cnv_timeout = setInterval(function(){
                    ...
                    var source, canvas;
                    if (time = new Date()
			.getTime(), $$.is("img")) source = new Image(), source.src = $$.attr("src")
...
clearInterval(fdt_cnv_timeout);
                ...
                function done(faces) {
                    for (var n = faces.length, data = [], i = 0; n > i; ++i) null !== options.confidence && faces[i].confidence <= options.confidence || (faces[i].positionX = position.left + faces[i].x
                        ...
                        , data.push(faces[i]));
                    data.time = new Date()
                        .getTime() - time, options.complete.apply($$, [data]);
                }
                var $$ = $(this)
                    , offset = $$.offset()
                    , position = $$.position()
